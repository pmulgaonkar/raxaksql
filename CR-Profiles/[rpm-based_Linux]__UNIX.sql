select add_profile_data ('[rpm-based Linux] UNIX DISA - Mission Critical Classified' ,'Profile Customized by CloudRaxak' ,'Linux-based OS','' ,'2015-05-26' ,'Profile by Cloud Raxak') from dual;
...
Stop the service if it is already running: 
 ...
 service autofs stop' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000526','C-45991r1_chk','To verify the "autofs" service is not running:
...
select add_rule_data ('default','V-38452','SV-50252r1_rule','default','low','The system package management tool must verify permissions on all files and directories associated with packages.','Permissions on system binaries and configuration files that are too generous could allow an unauthorized user to gain privileges that they should not have. The permissions set by the vendor should be maintained. Any deviations from this baseline should be investigated.','F-43398r1_fix','The RPM package management system can restore file access permissions of package files and directories. The following command will update permissions on files and directories with permissions different from what is expected by the RPM database: 
 ...
 rpm --setperms []' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000518','C-46008r1_chk','The following command will list which files and directories on the system have permissions different from what is expected by the RPM database: 
...
INFO: minlen should be >= 14 and the credits limit should be less or equal to 0 for the validation criteria in the rule.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000050','C-46029r1_chk','To check the minimum password length and to check the other settings required for the password authentication, run the command:
...
$ pwunconv; pwconv' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000053','C-46034r1_chk','To check the maximum password age, run the command: 
...
$ pwunconv; pwconv' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000051','C-46032r1_chk','To check the minimum password age, run the command: 
...
Add "dcredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of a digit in password.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000056','C-46037r1_chk','To check how many digits are required in a password, run the following command: 
...
gpgcheck=1' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000015','C-46043r1_chk','To determine whether "yum" has been configured to disable "gpgcheck" for any repos, inspect all files in "/etc/yum.repos.d" and ensure the following does not appear in any sections: 
...
If this check produces any unexpected output, investigate. ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000018','C-53727r1_chk','To find the location of the AIDE database file, run the following command:
...
Disable logon access to these accounts with the command: 
 ...
 passwd -l []' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000029','C-46052r2_chk','To obtain the system accounts which is unlocked run the following command: 
...
Note that any updates made to "/etc/pam.d/system-auth" and "/etc/pam.d/password-auth" may be overwritten by the "authconfig" program.  The "authconfig" program should not be used.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000061','C-46131r4_chk','To ensure the failed password attempt policy is configured correctly, run the following command: 
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000082','C-46068r3_chk','Check if IP forwarding for IPv4 is enabled on the system, by querying the following command:
...
$ install tipc /bin/true' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000127','C-46074r3_chk','If the system is configured to prevent the loading of the "tipc" kernel module, it will contain lines inside any file in "/etc/modprobe.d" or the deprecated "/etc/modprobe.conf". These lines instruct the module loading system to run another program (such as "/bin/true") upon a module "install" event. Run the following command to search for such lines in all files in "/etc/modprobe.d" and the deprecated "/etc/modprobe.conf": 
...
*.* :omrelp:[loghost.example.com]' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000136','C-46078r1_chk','To ensure logs are sent to a remote host, examine the file "/etc/rsyslog.conf". If using UDP, a line similar to the following should be present: 
...
or restart sysctl' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000088','C-46086r3_chk','Check if the system is configured to log Martian packets, by querying the following command:
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000089','C-46088r2_chk','Check if system is configured to reject IPv4 source-routed packets by default, by running the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000175','C-46092r1_chk','To determine if the system is configured to audit account changes, run the following command: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000176','C-46094r1_chk','To determine if the system is configured to audit account changes, run the following command: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000177','C-46096r1_chk','To determine if the system is configured to audit account changes, run the following command: 
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000095','C-46097r2_chk','Check if the system is configured to use TCP syncookies when experiencing a TCP SYN flood, by querying the following command:
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000096','C-46100r2_chk','Check if system is configured to use the reverse-path filter for IPv4 network traffic on all interfaces enabled
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000184','C-46101r2_chk','To determine if the system is configured to audit calls to the "chmod" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000185','C-46103r2_chk','To determine if the system is configured to audit calls to the "chown" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000186','C-46105r2_chk','To determine if the system is configured to audit calls to the "fchmod" system call, run the following command:
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000099','C-46106r3_chk','Check if IPv6 module is enabled or not using the following command :
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000187','C-46108r2_chk','To determine if the system is configured to audit calls to the "fchmodat" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000188','C-46110r2_chk','To determine if the system is configured to audit calls to the "fchown" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000189','C-46112r2_chk','To determine if the system is configured to audit calls to the "fchownat" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000190','C-46114r2_chk','To determine if the system is configured to audit calls to the "fremovexattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000191','C-46115r2_chk','To determine if the system is configured to audit calls to the "fsetxattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000192','C-46116r2_chk','To determine if the system is configured to audit calls to the "lchown" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000193','C-46117r2_chk','To determine if the system is configured to audit calls to the "lremovexattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000194','C-46119r2_chk','To determine if the system is configured to audit calls to the "lsetxattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000195','C-46121r2_chk','To determine if the system is configured to audit calls to the "removexattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000196','C-46123r2_chk','To determine if the system is configured to audit calls to the "setxattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000197','C-46124r1_chk','To verify that the audit system collects unauthorized file accesses, run the following commands: 
...
Add "ucredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of an uppercase character in a password.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000057','C-46127r1_chk','To check how many uppercase characters are required in a password, run the following command: 
...
Add "ocredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of a special character in passwords.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000058','C-46128r1_chk','To check how many special characters are required in a password, run the following command: 
...
Add "lcredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of a lowercase character in a password.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000059','C-46129r1_chk','To check how many lowercase characters are required in a password, run the following command: 
...
Note that any updates made to "/etc/pam.d/system-auth" will be overwritten by the "authconfig" program.  The "authconfig" program should not be used.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000062','C-46132r3_chk','Inspect the "password" section of "/etc/pam.d/system-auth", "/etc/pam.d/system-auth-ac", and other files in "/etc/pam.d" and ensure that the "pam_unix.so" module includes the argument "sha512".
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000200','C-46133r3_chk','To determine if the system is configured to audit calls to the "rmdir" system call, run the following command:
...
ENCRYPT_METHOD SHA512' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000063','C-46134r1_chk','Inspect "/etc/login.defs" and ensure the following line appears: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000202','C-46138r3_chk','To determine if the system is configured to audit execution of module management programs, run the following commands:
...
select add_rule_data ('default','V-38582','SV-50383r2_rule','default','medium','The xinetd service must be disabled if no network services utilizing it are enabled.','The xinetd service provides a dedicated listener service for some programs, which is no longer necessary for commonly-used network services. Disabling it ensures that these uncommon services are not running, and also prevents attacks against xinetd itself.','F-43530r2_fix','The "xinetd" service can be disabled with the following commands: 
 ...
 service xinetd stop' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000203','C-46140r2_chk','If network services are using the xinetd service, this is not applicable.
...
password --encrypted [password-hash]' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000068','C-46143r2_chk','To verify the boot loader password has been set and encrypted, run the following command: 
...
Mitigation:  If an enabled telnet daemon is configured to only allow encrypted sessions, such as with Kerberos or the use of encrypted network tunnels, the risk of exposing sensitive information is mitigated.','F-43537r1_fix','The "telnet" service can be disabled with the following command: 
 ...
 chkconfig telnet off' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000211','C-46147r3_chk','To check that the "telnet" service is disabled in system boot configuration, run the following command: 
...
$ service sshd restart' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000073','C-46150r1_chk','To check if the system login banner is compliant, run the following command: 
...
select add_rule_data ('default','V-38594','SV-50395r2_rule','default','high','The rshd service must not be running.','The rsh service uses unencrypted network communications, which means that data from the login session, including passwords and all other information transmitted during the session, can be stolen by eavesdroppers on the network.','F-43542r3_fix','The "rsh" service, which is available with the "rsh-server" package and runs as a service through xinetd, should be disabled. The "rsh" service can be disabled with the following command: 
 ...
 chkconfig rsh off' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000214','C-46152r2_chk','To check that the "rsh" service is disabled in system boot configuration, run the following command:
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000078','C-46153r2_chk','Check if the system is configured to implement virtual address space randomization, by querying the following command:
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000079','C-46155r3_chk','Check if the system is configured to limit the ability of processes to have simultaneous write and execute 
...
select add_rule_data ('default','V-38598','SV-50399r2_rule','default','high','The rexecd service must not be running.','The rexec service uses unencrypted network communications, which means that data from the login session, including passwords and all other information transmitted during the session, can be stolen by eavesdroppers on the network.','F-43546r3_fix','The "rexec" service, which is available with the "rsh-server" package and runs as a service through xinetd, should be disabled. The "rexec" service can be disabled with the following command: 
 ...
 chkconfig rexec off' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000216','C-46156r3_chk','To check that the "rexec" service is disabled in system boot configuration, run the following command:
...
Restart the vsftpd daemon.
 ...
 service vsftpd restart' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000348','C-46174r1_chk','If vstfpd service is not installed on the system, this rule is not applicable.
...
or restart sysctl.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000081','C-46159r2_chk','Check if system sends ICMPv4 redirects from any interface, by querying the following command:
...
select add_rule_data ('default','V-38602','SV-50403r2_rule','default','high','The rlogind service must not be running.','The rlogin service uses unencrypted network communications, which means that data from the login session, including passwords and all other information transmitted during the session, can be stolen by eavesdroppers on the network.','F-43549r3_fix','The "rlogin" service, which is available with the "rsh-server" package and runs as a service through xinetd, should be disabled. The "rlogin" service can be disabled with the following command: 
 ...
 chkconfig rlogin off' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000218','C-46158r3_chk',' 
...
$ service sshd restart' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000227','C-46165r1_chk','To check which SSH protocol version is allowed, run the following command: 
...
4. User account password  must be set or RSA key­pairs should be configured.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000237','C-46171r1_chk','To determine how the SSH daemon"s "PermitRootLogin" option is set, run the following command: 
...
$ service sshd restart
                        ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000240','C-46173r1_chk','To determine how the SSH daemon"s "Banner" option is set, run the following command: 
...
$ service sshd restart' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000241','C-46175r1_chk','To ensure users are not able to present environment daemons, run the following command: 
...
$ service sshd restart' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000243','C-46176r1_chk','Only FIPS-approved ciphers should be used. To verify that only FIPS-approved ciphers are in use, run the following command: 
...
If that is not the case, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38619','SV-50420r2_rule','default','medium','There must be no .netrc files on the system.','Unencrypted passwords for remote FTP servers may be stored in ".netrc" files. DoD policy requires passwords be encrypted in storage and not used in access scripts.','F-43569r2_fix','The ".netrc" files contain logon information used to auto-logon into FTP servers and reside in the user"s home directory. These files may contain unencrypted passwords to remote FTP servers making them susceptible to access by unauthorized users and should not be used. Any ".netrc" files should be removed.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000347','C-46179r3_chk','To check the system for the existence of any ".netrc" files, run the following command: 
...
$ service ntpd restart' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000248','C-46180r1_chk','Run the following command to determine the current status of the "ntpd" service: 
...
$ service ntp start
$ chkconfig ntpd on' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000247','C-46178r1_chk','To verify the "ntp" package is installed on the system, run the following command: 
...
1        5        cron.daily        run-parts --report /etc/cron.daily' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000138','C-46183r1_chk','Run the following commands to determine the current status of the "logrotate" configuration.
...
Contact LDAP administrator and configure LDAP over TLS/SSL.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000252','C-46184r1_chk','If the system does not use LDAP for authentication or account information, this is not applicable.
...
Then review the LDAP server and ensure TLS has been configured.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000253','C-46185r1_chk','If the system does not use LDAP for authentication or account information, this is not applicable.
...
select add_rule_data ('default','V-38628','SV-50429r2_rule','default','medium','The operating system must produce audit records containing sufficient information to establish the identity of any user/subject associated with the event.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43576r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 chkconfig --level 2345 auditd on' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000145','C-46186r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000258','C-46189r3_chk','If the GConf2 package is not installed, this is not applicable.
...
select add_rule_data ('default','V-38631','SV-50432r2_rule','default','medium','The operating system must employ automated mechanisms to facilitate the monitoring and control of remote access methods.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43580r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 chkconfig --level 2345 auditd on' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000148','C-46190r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
select add_rule_data ('default','V-38632','SV-50433r2_rule','default','medium','The operating system must produce audit records containing sufficient information to establish what type of events occurred.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43581r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 chkconfig --level 2345 auditd on' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000154','C-46191r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
$ service auditd reload' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000511','C-46020r1_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine if the system is configured to take appropriate action when disk errors occur:
...
UMASK 077' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000345','C-46205r1_chk','Verify the "umask" setting is configured correctly in the "/etc/login.defs" file by running the following command: 
...
umask 077' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000344','C-46207r1_chk','Verify the "umask" setting is configured correctly in the "/etc/profile" file by running the following command: 
...
select add_rule_data ('default','V-38653','SV-50454r1_rule','default','high','The snmpd service must not use a default password.','Presence of the default SNMP password enables querying of different system aspects and could result in unauthorized knowledge of the system.','F-43602r1_fix','Edit "/etc/snmp/snmpd.conf", remove default community string "public". Upon doing that, restart the SNMP service: 
 ...
 service snmpd restart' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000341','C-46213r1_chk','To ensure the default password is not set, run the following command: 

 grep -v "^" /etc/snmp/snmpd.conf| grep public

There should be no output. 
If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38654','SV-50455r2_rule','default','medium','Remote file systems must be mounted with the nosuid option.','NFS mounts should not present suid binaries to users. Only vendor-supplied suid executables should be installed to their default location on the local filesystem.','F-43603r1_fix','Add the "nosuid" option to the mount option column of "/etc/fstab" for the line which controls mounting of any NFS mounts.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000270','C-46214r3_chk','To verify the "nosuid" option is configured for all NFS mounts, run the following command: 
...
The requirement is 24 passwords.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000274','C-46219r1_chk','To verify the password reuse setting is compliant, run the following command: 
...
https://docs.redhat.com/docs/en-US/Red_Hat_Enterprise_Linux/6/html/Security_Guide/sect-Security_Guide-LUKS_Disk_Encryption.html' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000277','C-46222r1_chk','Determine if encryption must be used to protect data on the system. 
...
Configure the virus scanning software to perform scans dynamically on all accessed files. If this is not possible, configure the system to scan all altered files on the system on a daily basis. If the system processes inbound SMTP mail, configure the virus scanner to scan all received mail. ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000284','C-46226r2_chk','
...
$ aide --init' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000285','C-46227r1_chk','Inspect the system to determine if intrusion detection software has been installed. Verify the intrusion detection software is active by using selinux or aide tools.
...
* hard core 0' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000308','C-46235r2_chk','To verify that core dumps are disabled for all users, run the following command:
...
GATEWAY=[local LAN default gateway]' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000292','C-46242r2_chk','To verify that DHCP is not being used, examine the following file for each interface. 
...
If no line is returned, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38683','SV-50484r1_rule','default','low','All accounts on the system must have unique user or account names','Unique usernames allow for accountability on the system.','F-43632r1_fix','Change usernames, or delete accounts, so each has a unique name.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000296','C-46245r1_chk','Run the following command to check for duplicate account names: 
...
A documented site-defined number may be substituted for 10 in the above.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000319','C-46246r2_chk','Run the following command to ensure the "maxlogins" value is configured for all users on the system:
...
Make sure you configure it to persist after reboot.
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000320','C-46248r1_chk','Check if IPv6 module is enabled or not using the following command :
...
To display a banner, this setting must be enabled and then banner text must also be set.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000324','C-46250r3_chk','If the GConf2 package is not installed, this is not applicable.
...
When entering a warning banner that spans several lines, remember to begin and end the string with """. This command writes directly to the file "/var/lib/gdm/.gconf/apps/gdm/simple-greeter/%gconf.xml", and this file can later be edited directly if necessary.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000326','C-46252r3_chk','If the GConf2 package is not installed, this is not applicable.
...
A value of 35 is recommended. If a password is currently on the verge of expiration, then 35 days remain until the account is automatically disabled. However, if the password will not expire for another 60 days, then 95 days could elapse until the account would be automatically disabled. See the "useradd" man page for more information. Determining the inactivity timeout must be done with careful consideration of the length of a "normal" period of inactivity for users in the particular environment. Setting the timeout too low incurs support costs and also has the potential to impact availability of the system to legitimate users.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000334','C-46254r2_chk','To verify the "INACTIVE" setting, run the following command: 
...
password required pam_cracklib.so maxrepeat=3 ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000299','C-46255r1_chk','To check the maximum value for consecutive repeating characters, run the following command: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000302','C-46257r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
To set the sticky bit on a world-writable directory [], run the following command: 
 ...
 chmod +t [DIR]' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000336','C-46259r4_chk','To find world-writable directories that lack the sticky bit, run the following command for each local partition []: 
...
$ service xinetd  reload' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000338','C-46263r1_chk','Verify "tftp" is configured by with the "-s" option by running the following command: 
...
log_ftp_protocol=YES' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000339','C-46264r1_chk','Find if logging is applied to the ftp daemon. 
...
select add_rule_data ('default','V-38445','SV-50245r2_rule','default','medium','Audit log files must be group-owned by root.','If non-privileged users can write to audit logs, audit trails can be modified or destroyed.','F-43390r1_fix','Change the group owner of the audit log file(s) with the following command: 
 ...
 chgrp root []' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000522','C-46000r1_chk','Run the following command to check the group owner of the system audit log file(s): 
...
select add_rule_data ('default','V-38450','SV-50250r1_rule','default','medium','The /etc/passwd file must be owned by root.','The "/etc/passwd" file contains information about the users that are configured on the system. Protection of this file is critical for system security.','F-43395r1_fix','Run the below command to change the ownership of "/etc/passwd" to root:: 
 ...
 chown root /etc/passwd' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000039','C-46005r1_chk','To check the ownership of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38457','SV-50257r1_rule','default','medium','The /etc/passwd file must have mode 0644 or less permissive.','If the "/etc/passwd" file is writable by a group-owner or the world the risk of its compromise is increased. The file contains the list of accounts on the system and associated information, and protection of this file is critical for system security.','F-43397r1_fix','To properly set the permissions of "/etc/passwd", run the command: 
 ...
 chmod 0644 /etc/passwd' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000041','C-46007r1_chk','To check the permissions of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38458','SV-50258r1_rule','default','medium','The /etc/group file must be owned by root.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43403r1_fix','Run the below command to change the ownership of "/etc/group" to root:: 
 ...
 chown root /etc/group' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000042','C-46013r1_chk','To check the ownership of "/etc/group", run the command: 
...
select add_rule_data ('default','V-38459','SV-50259r1_rule','default','medium','The /etc/group file must be group-owned by root.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43404r1_fix','To change the group ownership of "/etc/group" to root, run the command: 
 ...
 chgrp root /etc/group' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000043','C-46014r1_chk','To check the group ownership of "/etc/group", run the command: 
...
select add_rule_data ('default','V-38461','SV-50261r1_rule','default','medium','The /etc/group file must have mode 0644 or less permissive.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43406r1_fix','To properly set the permissions of "/etc/group", run the command: 
 ...
 chmod 644 /etc/group' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000044','C-46015r1_chk','To check the permissions of "/etc/group", run the command: 
...
If any file in these directories is found to be group-writable or world-writable, correct its permission with the following command: 
 ...
 chmod go-w []' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000045','C-46019r4_chk','System-wide shared library files, which are linked to executables during process load time or run time, are stored in the following directories by default: 
...
If any file in these directories is found to be group-writable or world-writable, change its permission with the following command: 
 ...
 chmod go-w []' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000047','C-46024r3_chk','System executables are stored in the following directories by default: 
...
select add_rule_data ('default','V-38495','SV-50296r1_rule','default','medium','Audit log files must be owned by root.','If non-privileged users can write to audit logs, audit trails can be modified or destroyed.','F-43443r1_fix','Change the owner of the audit log files with the following command: 
 ...
 chown root []' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000384','C-46053r1_chk','Run the following command to check the owner of the system audit logs: 
...
select add_rule_data ('default','V-38498','SV-50299r1_rule','default','medium','Audit log files must have mode 0640 or less permissive.','If users can write to audit logs, audit trails can be modified or destroyed.','F-43445r1_fix','Change the mode of the audit log files with the following command: 
 ...
 chmod 0640 []' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000383','C-46055r1_chk','Run the following command to check the mode of the system audit logs: 
...
select add_rule_data ('default','V-38502','SV-50303r1_rule','default','medium','The /etc/shadow file must be owned by root.','The "/etc/shadow" file contains the list of local system accounts and stores password hashes. Protection of this file is critical for system security. Failure to give ownership of this file to root provides the designated owner with access to sensitive information which could weaken the system security posture.','F-43449r1_fix','Run the below command to change the ownership of "/etc/shawdow" to root:: 
 ...
 chown root /etc/shadow' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000033','C-46059r1_chk','To check the ownership of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38504','SV-50305r1_rule','default','medium','The /etc/shadow file must have mode 0000.','The "/etc/shadow" file contains the list of local system accounts and stores password hashes. Protection of this file is critical for system security. Failure to give ownership of this file to root provides the designated owner with access to sensitive information which could weaken the system security posture.','F-43451r1_fix','To properly set the permissions of "/etc/shadow", run the command: 
 ...
 chmod 0000 /etc/shadow' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000035','C-46061r2_chk','To check the permissions of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38579','SV-50380r1_rule','default','medium','The system boot loader configuration file(s) must be owned by root.','Only root should be able to modify important boot parameters.','F-43527r1_fix','The file "/etc/grub.conf" should be owned by the "root" user to prevent destruction or modification of the file. To change the owner of "/etc/grub.conf" to root, run the command: 
 ...
 chown root /etc/grub.conf' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000065','C-46137r1_chk','To check the ownership of "/etc/grub.conf", run the command: 
...
select add_rule_data ('default','V-38581','SV-50382r1_rule','default','medium','The system boot loader configuration file(s) must be group-owned by root.','The "root" group is a highly-privileged group. Furthermore, the group-owner of this file should not have any access privileges anyway.','F-43529r1_fix','The file "/etc/grub.conf" should be group-owned by the "root" group to prevent destruction or modification of the file. To change the group owner of "/etc/grub.conf" to root, run the command: 
 ...
 chgrp root /etc/grub.conf' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000066','C-46139r1_chk','To check the group ownership of "/etc/grub.conf", run the command: 
...
Change the permission of /etc/cron.allow to 600 and change the user and group ownership to root.
 ...
 chgrp root /etc/cron.allow
                    ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000276','','
...
If any account has reserved uid, not allowing that account to login.
   ','F-56055','
If any user has reserved uid take necessary action since this improper application of permissions can lead to security risks. Assign a different uid to those particular accounts other than system accounts to fix this.
   ' , '[rpm-based Linux] UNIX DISA - Mission Critical Classified','Profile Customized by CloudRaxak','RHEL-06-000276','','
Check whether reserved UIDs are assigned to system accounts by comparing the uid"s from the /etc/passwd file with reserved uid"s in /usr/share/doc/setup*/uidgid. This file contains list of reserved UID and GID.
