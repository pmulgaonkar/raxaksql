select add_profile_data ('[debian-based Linux] DISA - Mission Critical Classified (Full)' ,'Profile Customized by CloudRaxak' ,'Linux-based OS','' ,'2015-05-26' ,'Profile by Cloud Raxak') from dual;
...
Stop the service if it is already running: 
 ...
 service autofs stop' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000526','C-45991r1_chk','To verify the "autofs" service is not running:
...
and run the following command to update the grub.
 ...
 update-grub
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000098','C-46104r2_chk','Check if IPv6 module is enabled or not using the following command :
...
Make sure all your iptables configuration is persistent.
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000523','C-45999r2_chk','Check if IPv6 module is enabled or not using the following command : 
...
select add_rule_data ('default','V-38669','SV-50470r1_rule','default','low','The postfix service must be enabled for mail delivery.','Local mail delivery is essential to some system maintenance and notification tasks.','F-43618r1_fix','The Postfix mail transfer agent is used for local mail delivery within the system. The default configuration only listens for connections to the default SMTP port (port 25) on the loopback interface (127.0.0.1). It is recommended to leave this service enabled for local mail delivery. The "postfix" service can be enabled with the following command: 
 ...
 service postfix start' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000287','C-46230r1_chk','Run the following command to determine the current status of the "postfix" service:
...
select add_rule_data ('default','V-38446','SV-50246r1_rule','default','medium','The mail system must forward all mail for root to one or more system administrators.','A number of system services utilize email messages sent to the root user to notify system administrators of active or impending issues.  These messages must be forwarded to at least one monitored email address.','F-43391r1_fix','Set up an alias for root that forwards to a monitored email address:
 ...
 newaliases' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000521','C-46001r1_chk','Find the list of alias maps used by the Postfix mail server:
...
inet_interfaces = localhost' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000249','C-46182r2_chk','If the system is an authorized mail relay host, this is not applicable. 
...
If the file that has changed was not expected to then refresh from distribution media or online repositories.
 ...
 apt-get -y install --reinstall []' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000519','C-46002r3_chk','The following command will list which files on the system have file hashes different from what is expected by the Ubuntu package management tool"s database.
...
select add_rule_data ('default','V-38460','SV-50260r1_rule','default','low','The NFS server must not have the all_squash option enabled.','The "all_squash" option maps all client requests to a single anonymous uid/gid on the NFS server, negating the ability to track file access by user ID.','F-43405r1_fix','Remove any instances of the "all_squash" option from the file "/etc/exports".  Restart the NFS daemon for the changes to take effect.
 ...
 service nfs-kernel-server restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000515','C-46016r1_chk','If the NFS server is read-only, in support of unrestricted access to organizational content, this is not applicable.
...
If auditd package is not installed then rule will install it before fixing changes.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000509','C-46026r1_chk','Verify the audispd plugin is active:
...
INFO: minlen should be >= 14 and the credits limit should be less or equal to 0 for the validation criteria in the rule.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000050','C-46029r1_chk','To check the minimum password length and to check the other settings required for the password authentication, run the command:
...
select add_rule_data ('default','V-38476','SV-50276r3_rule','default','high','Vendor-provided cryptographic certificates must be installed to verify the integrity of system software.','The Red Hat GPG keys are necessary to cryptographically verify packages are from Red Hat. ','F-43421r3_fix','To ensure the system can cryptographically verify base software packages installation. Use the command:
 apt-key update' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000008','C-46031r3_chk','To ensure that the keys are installed, run:
...
select add_rule_data ('default','V-38478','SV-50278r2_rule','default','low','The Red Hat Network Service (rhnsd) service must not be running, unless using RHN or an RHN Satellite.','Although systems management and patching is extremely important to system security, management by a system outside the enterprise enclave is not desirable for some environments. However, if the system is being managed by RHN or RHN Satellite Server the "rhnsd" daemon can remain on.','F-43423r2_fix','The Red Hat Network service automatically queries Red Hat Network servers to determine whether there are any actions that should be executed, such as package updates. This only occurs if the system was registered to an RHN server or satellite and managed as such. The "rhnsd" service can be disabled with the following commands: 
 ...
 service rhnsd stop' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000009','C-46033r2_chk','Run the following command to verify "rhnsd" is disabled through current runtime configuration: 
...
$ pwunconv; pwconv' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000053','C-46034r1_chk','To check the maximum password age, run the command: 
...
$ pwunconv; pwconv' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000051','C-46032r1_chk','To check the minimum password age, run the command: 
...
$ pwunconv; pwconv' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000054','C-46035r1_chk','To check the password warning age, run the command: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000507','C-46041r2_chk','Verify the value associated with the "PrintLastLog" keyword in /etc/ssh/sshd_config:
...
At console or tty logon, a user must be presented with the last successful login date and time.  The "hushlogin" file determines whether this occurs.','F-43432r1_fix','Remove any "hushlogin" files from the system:
 ...
 rm /etc/hushlogins
 rm ~<userid>/.hushlogin' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000506','C-46042r1_chk','Verify there are no "hushlogin" files active on the system:
 ls -l /etc/hushlogins
...
select add_rule_data ('default','V-38486','SV-50287r1_rule','default','medium','The operating system must conduct backups of system-level information contained in the information system per organization defined frequency to conduct backups that are consistent with recovery time and recovery point objectives.','Operating system backup is a critical step in maintaining data assurance and availability. System-level information includes system-state information, operating system and application software, and licenses. Backups must be consistent with organizational recovery time and recovery point objectives.','F-43434r1_fix','Procedures to back up OS data from the system must be established and executed. Commercial and open-source products are also available.

Implement a process whereby OS data is backed up from the system in accordance with local policies.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000505','C-46044r1_chk','Ask an administrator if a process exists to back up OS data from the system, including configuration data. 
...
$ grep -rwi AllowUnauthenticated /etc/apt/* | grep -v ":\s*"
','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38488','SV-50289r1_rule','default','medium','The operating system must conduct backups of user-level information contained in the operating system per organization defined frequency to conduct backups consistent with recovery time and recovery point objectives.','Operating system backup is a critical step in maintaining data assurance and availability. User-level information is data generated by information system and/or application users. Backups shall be consistent with organizational recovery time and recovery point objectives.','F-43435r1_fix','Procedures to back up user data from the system must be established and executed. Commercial and open-source products are also available.

Implement a process whereby user data is backed up from the system in accordance with local policies.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000504','C-46045r1_chk','Ask an administrator if a process exists to back up user data from the system. 
...
select add_rule_data ('default','V-38489','SV-50290r1_rule','default','medium','A file integrity tool aide must be installed.','The AIDE package must be installed if it is to be available for integrity checking.','F-43436r1_fix','Install the aide, aide-common packages with the commands: 
 ...
 apt-get -y install aide-common' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000016','C-46046r1_chk','Run the following command to determine if the "aide", "aide-common" packages are installed: 
...
If this check produces any unexpected output, investigate. ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000018','C-53727r1_chk','To find the location of the AIDE database file, run the following command:
...
' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000019','C-46048r1_chk','The existence of the file named "hosts.equiv" or ".rhosts" inside a user home directory indicates the presence of an Rsh trust relationship, use the command:
...
Note:  Virtual console entries are not limited to those listed above.  Any lines starting with "vc/" followed by numerals should be removed.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000027','C-46049r1_chk','To check for virtual console entries which permit root login, run the following command: 
...
Note:  Serial port entries are not limited to those listed above.  Any lines starting with "ttyS" followed by numerals should be removed' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000028','C-46051r1_chk','To check for serial port entries which permit root login, run the following command: 
...
Disable logon access to these accounts with the command: 
 ...
 passwd -l []' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000029','C-46052r2_chk','To obtain the system accounts which is unlocked run the following command:
...
Note that any updates made to "/etc/pam.d/common-auth" may be overwritten by the "authconfig" program.  The "authconfig" program should not be used.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000061','C-46131r4_chk','To ensure the failed password attempt policy is configured correctly, run the following command: 
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000082','C-46068r3_chk','Check if IP forwarding for IPv4 is enabled on the system, by querying the following command: 
...
Make sure you configure the rules to persist after reboot.
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000117','C-46069r2_chk','Run the following command to determine the current status of the Firewall:
...
Make sure all your iptables configuration is persistent.
   ...
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000120','C-46070r1_chk','Run the following command to determine the current status of the Firewall:
 iptables -L
...
*.* :omrelp:[loghost.example.com]' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000136','C-46078r1_chk','To ensure logs are sent to a remote host, examine the file "/etc/rsyslog.conf". If using UDP, a line similar to the following should be present: 
...
*.* :omrelp:[loghost.example.com]' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000137','C-46269r1_chk','To ensure logs are sent to a remote host, examine the file "/etc/rsyslog.conf". If using UDP, a line similar to the following should be present: 
...
Now, to get recently added audit rules into effect run the following command: 
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000167','C-46080r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000083','C-46081r3_chk','Check if system is configured to reject IPv4 source-routed packets on any interface, by querying the following command:
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000084','C-46082r2_chk','Check if the system is configured to reject ICMPv4 redirect packets, by querying the following command:
...
Now, to get recently added audit rules into effect run the following command: 
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000169','C-46083r3_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000086','C-46084r2_chk','To check if system accepts ICMPv4 secure redirect packets on any interface, run the following command:
...
Now, to get recently added audit rules into effect run the following command: 
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000171','C-46085r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000088','C-46086r3_chk','Check if the system is configured to log Martian packets, by querying the following command:
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000089','C-46088r2_chk','Check if system is configured to reject IPv4 source-routed packets by default, by running the following command:
...
Now, to get recently added audit rules into effect run the following command: 
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000173','C-46087r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000174','C-46090r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000090','C-46089r2_chk','Check if the system is configured to reject ICMPv4 secure redirect packets by default, by querying the following command: 
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000091','C-46091r2_chk','Check if the system is configured to ignore ICMPv4 redirect messages by default, by querying the following command: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000175','C-46092r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000092','C-46093r2_chk','The status of the "net.ipv4.icmp_echo_ignore_broadcasts" kernel parameter can be queried by running the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000176','C-46094r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000093','C-46095r2_chk','The status of the "net.ipv4.icmp_ignore_bogus_error_responses" kernel parameter can be queried by running the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000177','C-46096r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000095','C-46097r2_chk','Check if the system is configured to use TCP syncookies when experiencing a TCP SYN flood, by querying the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000182','C-46098r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000183','C-46099r1_chk','To determine if the system is configured to audit changes to its MAC configuration files, run the following command:
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000096','C-46100r2_chk','Check if system is configured to use the reverse-path filter for IPv4 network traffic on all interfaces enabled
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000184','C-46101r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000097','C-46102r2_chk','Check if the system is configured to use TCP syncookies when experiencing a TCP SYN flood, by querying the following command: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000185','C-46103r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000186','C-46105r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000099','C-46106r3_chk','Check if IPv6 module is enabled or not using the following command :
...
and configure the same to deny-all packets except coming from CloudRaxak.
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000103','C-46107r3_chk','Check if IPv6 module is enabled or not using the following command :
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000187','C-46108r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000106','C-46109r3_chk','Check if IPv6 module is enabled or not using the following command :
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000188','C-46110r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
To make this configuration is persistent, run following command.
...
' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000107','C-46111r3_chk','Check if IPv6 module is enabled or not using the following command :
...
make sure ip_Forwarding is enabled
 ...
 sysctl "net.ipv6.conf" | grep "\.forwarding = 1"
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000189','C-46112r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
and configure the same to deny-all packets except coming from CloudRaxak.
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000113','C-46113r2_chk','Run the following command to determine the current status of the Firewall:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000190','C-46114r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000191','C-46115r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000192','C-46116r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000193','C-46117r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000116','C-46118r2_chk','Run the following command to determine the current status of the Firewall:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000194','C-46119r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000195','C-46121r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000196','C-46123r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000197','C-46124r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000199','C-46126r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Add "ucredit=-1" after pam_cracklib.so in /etc/pam.d/common-password file to require use of an uppercase character in a password.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000057','C-46127r1_chk','To check how many uppercase characters are required in a password, run the following command: 
...
Add "lcredit=-1" after pam_cracklib.so in /etc/pam.d/common-password file to require use of a lowercase character in password.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000059','C-46129r1_chk','To check how many lowercase characters are required in a password, run the following command: 
...
The pam_cracklib module"s "difok" parameter controls requirements for usage of different characters during a password change. Add "difok=[]" after pam_cracklib. So to require differing characters when changing passwords, substituting [NUM] appropriately. The requirement is greater or equal to 4.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000060','C-46130r1_chk','To check how many characters must differ during a password change, run the following command: 
...
Note that any updates made to "/etc/pam.d/common-password" will be overwritten by the "authconfig" program.  The "authconfig" program should not be used.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000062','C-46132r3_chk','Inspect the "password" section of "/etc/pam.d/common-password", and other files in "/etc/pam.d" and ensure that the "pam_unix.so" module includes the argument "sha512".
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000200','C-46133r3_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
ENCRYPT_METHOD SHA512' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000063','C-46134r1_chk','Inspect "/etc/login.defs" and ensure the following line appears: 
...
crypt_style = sha512' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000064','C-46135r1_chk','Inspect "/etc/libuser.conf" and ensure the following line appears in the "[]" section: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000201','C-46136r1_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000202','C-46138r3_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
select add_rule_data ('default','V-38582','SV-50383r2_rule','default','medium','The xinetd service must be disabled if no network services utilizing it are enabled.','The xinetd service provides a dedicated listener service for some programs, which is no longer necessary for commonly-used network services. Disabling it ensures that these uncommon services are not running, and also prevents attacks against xinetd itself.','F-43530r2_fix','The "xinetd" service can be disabled with the following commands: 
 ...
 service xinetd stop' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000203','C-46140r2_chk','If network services are using the xinetd service, this is not applicable.
...
select add_rule_data ('default','V-38584','SV-50385r1_rule','default','low','The xinetd service must be uninstalled if no network services utilizing it are enabled.','Removing the "xinetd" package decreases the risk of the xinetd service"s accidental (or intentional) activation.','F-43532r1_fix','The "xinetd" package can be uninstalled with the following command: 
 ...
 dpkg -r --force-all xinetd' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000204','C-46142r1_chk','If network services are using the xinetd service, 
...
And update the grub
 update-grub' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000069','C-46145r1_chk','To verify the single-user mode authentication is disabled, check "init=/bin/bash", "init=/bin/sh" parameters existancy inkernel parameter line in grub file.
...
Mitigation:  If the telnet-server package is configured to only allow encrypted sessions, such as with Kerberos or the use of encrypted network tunnels, the risk of exposing sensitive information is mitigated.','F-43535r1_fix','The "telnetd" package can be uninstalled with the following command: 
 ...
 dpkg -r --force-all telnetd' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000206','C-46144r1_chk','Run the following command to determine if the "telnetd" package is installed: 

 dpkg -s telnetd


If the package is installed, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38588','SV-50389r1_rule','default','medium','The system must not permit interactive boot.','Using interactive boot, the console user could disable auditing, firewalls, or other services, weakening system security.','F-43536r1_fix','This rule is not applicable.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000070','C-46146r1_chk','This rule is not applicable.','Customized rule by Cloud Raxak') from dual;
...
$ service openbsd-inetd restart
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000211','C-46147r3_chk','To check that the "telnet" service is disabled, follow the folllowing step:
Check the telnet configuration file:
$ grep -w /usr/sbin/in.telnetd /etc/xinetd.d/*
...
ctrl+a x' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000071','C-46148r1_chk','Run the following command to determine if the "screen" package is installed: 
...
select add_rule_data ('default','V-38591','SV-50392r1_rule','default','high','The rsh-server package must not be installed.','The "rsh-server" package provides several obsolete and insecure network services. Removing it decreases the risk of those services" accidental (or intentional) activation.','F-43539r1_fix','The "rsh-server" package can be uninstalled with the following command: 
 ...
 dpkg -r --force-all rsh-server' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000213','C-46149r1_chk','Run the following command to determine if the "rsh-server" package is installed: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000073','C-46150r1_chk','To check if the system login banner is compliant, run the following command: 
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000078','C-46153r2_chk','Check if the system is configured to implement virtual address space randomization, by querying the following command:
...
Restart the vsftpd daemon.
 ...
 service vsftpd restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000348','C-46174r1_chk','If vstfpd service is not installed on the system, this rule is not applicable.
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000080','C-46157r2_chk','Check if the system is configured to ignore ICMPv4 redirects by default, by querying the following command:
...
or restart sysctl.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000081','C-46159r2_chk','Check if system sends ICMPv4 redirects from any interface, by querying the following command:
...
(Since ypserv is nis in ubuntu so there is no separate package for ypserv. Hence, we are removing nis package.)
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000220','C-46161r1_chk','Run the following command to determine if the "nis" package is installed: 
...
select add_rule_data ('default','V-38604','SV-50405r2_rule','default','medium','The ypbind service must not be running.','Disabling the "ypbind" service ensures the system is not acting as a client in a NIS or NIS+ domain.','F-43552r2_fix','The "ypbind" service, which allows the system to act as a client in a NIS or NIS+ domain, should be disabled. The "ypbind" service can be disabled with the following commands: 
 ...
 service ypbind stop
 sysv-rc-conf --level 0123456S ypbind off' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000221','C-46162r2_chk','Run the following command to verify "ypbind" is disabled through current runtime configuration: 
...
select add_rule_data ('default','V-38605','SV-50406r2_rule','default','medium','The cron service must be running.','Due to its usage for maintenance and security-supporting tasks, enabling the cron daemon is essential.','F-43553r2_fix','The "cron" service is used to execute commands at preconfigured times. It is required by almost all systems to perform necessary maintenance tasks, such as notifying root of system activity. The "cron" service can be enabled with the following commands: 
 ...
 service cron start' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000224','C-46163r1_chk','Run the following command to determine the current status of the "crond" service: 
...
If tftpd is not installed the check for tftpd-hpa package, to remove run following command:
 ...
 dpkg -r --force-all tftpd-hpa
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000222','C-46164r1_chk','Run the following command to determine if the "tftpd" package is installed: 
...
$ service ssh restart
' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000227','C-46165r1_chk','To check which SSH protocol version is allowed, run the following command: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000231','C-46168r1_chk','To ensure the SSH idle timeout will occur when the "ClientAliveCountMax" is set, run the following command: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000230','C-46167r1_chk','Run the following command to see what the timeout interval is: 
...
The "tftpd-hpa" service can be disabled with the following commands:
 ...
 service tftpd-hpa stop
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000223','C-46166r2_chk','To check that the "TFTP" service is disabled, follow the folllowing step:
Check the tftp configuration file:
$ sudo grep -w /usr/sbin/in.tftpd /etc/xinetd.d/*
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000234','C-46169r1_chk','To determine how the SSH daemon"s "IgnoreRhosts" option is set, run the following command: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000236','C-46170r1_chk','To determine how the SSH daemon"s "HostbasedAuthentication" option is set, run the following command: 
...
4. User account password  must be set or RSA key­pairs should be configured.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000237','C-46171r1_chk','To determine how the SSH daemon"s "PermitRootLogin" option is set, run the following command: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000239','C-46172r1_chk','To determine how the SSH daemon"s "PermitEmptyPasswords" option is set, run the following command: 
...
Another section contains information on how to create an appropriate system-wide warning banner.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000240','C-46173r1_chk','To determine how the SSH daemon"s "Banner" option is set, run the following command: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000241','C-46175r1_chk','To ensure users are not able to present environment daemons, run the following command: 
...
$ service ssh restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000243','C-46176r1_chk','Only FIPS-approved ciphers should be used. To verify that only FIPS-approved ciphers are in use, run the following command: 
...
select add_rule_data ('default','V-38618','SV-50419r2_rule','default','low','The avahi service must be disabled.','Because the Avahi daemon service keeps an open network port, it is subject to network attacks. Its functionality is convenient but is only appropriate if the local network can be trusted.','F-43567r2_fix','The "avahi-daemon" service can be disabled with the following commands: 
 ...
 service avahi-daemon stop' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000246','C-46177r1_chk','Run the following command to verify "avahi-daemon" is disabled through current runtime configuration:
...
If the service is running, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38619','SV-50420r2_rule','default','medium','There must be no .netrc files on the system.','Unencrypted passwords for remote FTP servers may be stored in ".netrc" files. DoD policy requires passwords be encrypted in storage and not used in access scripts.','F-43569r2_fix','The ".netrc" files contain logon information used to auto-logon into FTP servers and reside in the user"s home directory. These files may contain unencrypted passwords to remote FTP servers making them susceptible to access by unauthorized users and should not be used. Any ".netrc" files should be removed.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000347','C-46179r3_chk','To check the system for the existence of any ".netrc" files, run the following command: 
...
$ service ntp restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000248','C-46180r1_chk','To verify the "ntpd" package is installed on the system, run the following command: 
...
$ service ntp start
$ sysv-rc-conf --level 0123456S ntp on' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000247','C-46178r1_chk','To verify the "ntpd" package is installed on the system, run the following command: 
...
1        5        cron.daily        run-parts --report /etc/cron.daily' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000138','C-46183r1_chk','Run the following commands to determine the current status of the "logrotate" configuration.
...
Contact LDAP administrator and configure LDAP over TLS/SSL.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000252','C-46184r1_chk','If the system does not use LDAP for authentication or account information, this is not applicable.
...
Then review the LDAP server and ensure TLS has been configured.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000253','C-46185r1_chk','If the system does not use LDAP for authentication or account information, this is not applicable.
...
select add_rule_data ('default','V-38628','SV-50429r2_rule','default','medium','The operating system must produce audit records containing sufficient information to establish the identity of any user/subject associated with the event.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43576r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 sysv-rc-conf --level 2345 auditd on' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000145','C-46186r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
Set the idle time-out value for global users with the follwoing command:
 ...
 sudo -u UserName -H dbus-launch gsettings set org.gnome.desktop.screensaver lock-delay 300' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000257','C-46188r3_chk','If the system is NON-GUI based system, this is not applicable.
...
' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000258','C-46189r3_chk','If the system is NON-GUI based system, this is not applicable.
...
select add_rule_data ('default','V-38631','SV-50432r2_rule','default','medium','The operating system must employ automated mechanisms to facilitate the monitoring and control of remote access methods.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43580r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 sysv-rc-conf --level 2345 auditd on' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000148','C-46190r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
sysv-rc-conf --level 2345 auditd on' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000154','C-46191r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000165','C-46194r2_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
$ service auditd reload' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000159','C-46195r1_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine how many logs the system is configured to retain after rotation: 
...
$ service auditd reload' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000160','C-46192r1_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine how much data the system will retain in each audit log file:
...
$ service auditd reload' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000161','C-46193r3_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine if the system is configured to rotate logs when they reach their maximum size:
...
Set this value to "single" to cause the system to switch to single-user mode for corrective action. Acceptable values also include "suspend" and "halt". For certain systems, the need for availability outweighs the need to log all actions, and a different setting should be determined. Details regarding all possible values for [ACTION] are described in the "auditd.conf" man page. ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000163','C-54997r2_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine if the system is configured to either suspend, switch to single-user mode, or halt when disk space has run low:
...
$ service auditd reload' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000511','C-46020r1_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine if the system is configured to take appropriate action when disk errors occur:
...
Activate the locking screen for global users by following command.
 sudo -u username -H dbus-launch gsettings set org.gnome.desktop.screensaver lock-enabled "true"' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000259','C-46198r3_chk','If the system is NON-GUI based system, this is not applicable. 
...
' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000260','C-46199r4_chk','If the system is NON-GUI based system, this is not applicable. 
...
select add_rule_data ('default','V-38640','SV-50441r2_rule','default','low','The automatically generate crash reports for debugging(apport) service must not be running.','Mishandling crash data could expose sensitive information about vulnerabilities in software executing on the local machine, as well as sensitive information from within a process"s address space or registers.','F-43589r2_fix','. The "apport" service can be disabled with the following commands: 
 service apport stop
 sysv-rc-conf --level 0123456S apport off' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000261','C-46200r1_chk','Run the following command to verify "apport" is disabled through current runtime configuration: 
...
select add_rule_data ('default','V-38641','SV-50442r2_rule','default','low','The atd service must be disabled.','The "atd" service could be used by an unsophisticated insider to carry out activities outside of a normal login session, which could complicate accountability. Furthermore, the need to schedule tasks with "at" or "batch" is not common.','F-43590r2_fix','The "at" and "batch" commands can be used to schedule tasks that are meant to be executed only once. This allows delayed execution in a manner similar to cron, except that it is not recurring. The daemon "atd" keeps track of tasks scheduled via "at" and "batch", and executes them at the specified time. The "atd" service can be disabled with the following commands: 
 ...
 service atd stop' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000262','C-46201r2_chk','If the system uses the "atd" service, this is not applicable.
...
select add_rule_data ('default','V-38644','SV-50445r2_rule','default','low','The ntpdate service must not be running.','The "ntpdate" service may only be suitable for systems which are rebooted frequently enough that clock drift does not cause problems between reboots. In any event, the functionality of the ntpdate service is now available in the ntpd program and should be considered deprecated.','F-43593r2_fix','The functionality of the ntpdate is now available as ntp daemon so it is considered deprecated hence remove the ntpdate package.
 ...
 apt-get -y purge ntpdate' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000265','C-46204r1_chk','Run the following command to verify "ntpdate" is installed on the system.
...
UMASK 077' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000345','C-46205r1_chk','Verify the "umask" setting is configured correctly in the "/etc/login.defs" file by running the following command:
...
umask 077' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000344','C-46207r1_chk','Verify the "umask" setting is configured correctly in the "/etc/profile" file by running the following command:
...
select add_rule_data ('default','V-38648','SV-50449r2_rule','default','low','The qpidd service must not be running.','The qpidd service is automatically installed when the "base" package selection is selected during installation. The qpidd service listens for network connections which increases the attack surface of the system. If the system is not intended to receive AMQP traffic then the "qpidd" service is not needed and should be disabled or removed.','F-43597r2_fix','The "qpidd" service provides high speed, secure, guaranteed delivery services. It is an implementation of the Advanced Message Queuing Protocol. By default the qpidd service will bind to port 5672 and listen for connection attempts. The "qpidd" service can be disabled with the following commands: 
 ...
 sysv-rc-conf --level 0123456S qpidd off
 service qpidd stop' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000267','C-46208r2_chk','Run the following command to verify "qpidd" is disabled through current runtime configuration: 

 service qpidd status

If the service is disabled the command will return the following output: 

qpidd is stoppedTo check that the "qpidd" service is disabled in system boot configuration, run the following command: 

 sysv-rc-conf --list qpidd
...
"qpidd" 0:off 1:off 2:off 3:off 4:off 5:off 6:off S:off
...
umask 077' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000343','C-46209r1_chk','Verify the "umask" setting is configured correctly in the "/etc/csh.cshrc" file by running the following command: 
...
umask 077' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000342','C-46211r1_chk','Verify the "umask" setting is configured correctly in the "/etc/bash.bashrc" file by running the following command:
...
select add_rule_data ('default','V-38653','SV-50454r1_rule','default','high','The snmpd service must not use a default password.','Presence of the default SNMP password enables querying of different system aspects and could result in unauthorized knowledge of the system.','F-43602r1_fix','Edit "/etc/snmp/snmpd.conf", remove default community string "public". Upon doing that, restart the SNMP service: 
 ...
 service snmpd restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000341','C-46213r1_chk','To ensure the default password is not set, run the following command: 

 grep -v "^" /etc/snmp/snmpd.conf| grep public

There should be no output. 
If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38654','SV-50455r2_rule','default','medium','Remote file systems must be mounted with the nosuid option.','NFS mounts should not present suid binaries to users. Only vendor-supplied suid executables should be installed to their default location on the local filesystem.','F-43603r1_fix','Add the "nosuid" option to the mount option column of "/etc/fstab" for the line which controls mounting of any NFS mounts.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000270','C-46214r3_chk','To verify the "nosuid" option is configured for all NFS mounts, run the following command: 
...
[]" and then run mount [devicename] to effect the changes.]
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000271','C-46216r1_chk','To verify that binaries cannot be directly executed from removable media, run the following command: 
...
Requiring samba clients such as "smbclient" to use packet signing ensures they can only communicate with servers that support packet signing.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000272','C-46217r1_chk','To verify that Samba clients running smbclient must use packet signing, run the following command:
...
See the "mount.cifs(8)" man page for more information. A Samba client should only communicate with servers who can support SMB packet signing.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000273','C-46218r4_chk','If Samba is not in use, this is not applicable.
...
The requirement is 24 passwords.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000274','C-46219r1_chk','To verify the password reuse setting is compliant, run the following command: 
...
$ /etc/init.d/snmpd reload' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000340','C-46215r1_chk','To ensure only SNMPv3 or newer is used, run the following command: 

$ grep "v1\|v2c\|com2sec" /etc/snmp/snmpd.conf | grep -v "^"

There should be no output. 
If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38661','SV-50462r1_rule','default','low','The operating system must protect the confidentiality and integrity of data at rest. ','The risk of a system"s physical compromise, particularly mobile systems such as laptops, places its data at risk of compromise. Encrypting this data mitigates the risk of its loss if the system is lost.','F-43610r1_fix','Ubuntu supports partition encryption through the Linux Unified Key Setup-on-disk-format(LUKS) technology. The easiest way to encrypt a partition is during installation time. 
...
Consult the Ubuntu documentation for detailed information on encrypting partitions using LUKS.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000276','C-46221r1_chk','Determine if encryption must be used to protect data on the system. 
If encryption must be used and is not employed, this is a finding.','Customized rule by Cloud Raxak') from dual;
...
Consult the Ubuntu documetation for detailed information on encrypting partitions using LUKS.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000277','C-46222r1_chk','Determine if encryption must be used to protect data on the system. 
If encryption must be used and is not employed, this is a finding.','Customized rule by Cloud Raxak') from dual;
...
Configure the virus scanning software to perform scans dynamically on all accessed files. If this is not possible, configure the system to scan all altered files on the system on a daily basis. If the system processes inbound SMTP mail, configure the virus scanner to scan all received mail. ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000284','C-46226r2_chk','Check if "McAfeeVSEForLinux" is installed on the system,
...
$ aideinit ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000285','C-46227r1_chk','Inspect the system to determine if intrusion detection software has been installed. Verify the intrusion detection software is active by using apparmor or aide tools.
...
AIDE can be executed periodically through other means; this is merely one example.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000306','C-46229r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
select add_rule_data ('default','V-38671','SV-50472r1_rule','default','medium','The sendmail package must be removed.','The sendmail software was not developed with security in mind and its design prevents it from being effectively contained by SELinux. Postfix should be used instead.','F-43620r1_fix','Sendmail is not the default mail transfer agent and is not installed by default. The "sendmail" package can be removed with the following command: 
 ...
 dpkg -r --force-all sendmail' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000288','C-46231r1_chk','Run the following command to determine if the "sendmail" package is installed: 
...
select add_rule_data ('default','V-38672','SV-50473r2_rule','default','low','The netconsole module must be disabled unless required.','The "netconsole" module is not necessary unless there is a need to debug kernel panics, which is not common.','F-43622r2_fix','The "netconsole" kernel module, which logs kernel printk messages over UDP to a syslog server. This allows debugging of problems where disk logging fails and serial consoles are impractical. The "netconsole" module can be disabled with the following commands: 
 ...
 rmmod netconsole ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000289','C-46233r1_chk','To check that the "netconsole" module is disabled, run the following command: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000307','C-46232r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
To reflect the changes reboot the system.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000290','C-46234r1_chk','To verify x-windows is disable check following parameter in /etc/default/grub file:
GRUB_CMDLINE_LINUX="text"
GRUB_TERMINAL=console
...
* hard core 0' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000308','C-46235r2_chk','To verify that core dumps are disabled for all users, run the following command:
...
select add_rule_data ('default','V-38676','SV-50477r2_rule','default','low','The xorg-x11-server-common (X Windows) package must not be installed, unless required.','Unnecessary packages should not be installed to decrease the attack surface of the system.','F-43625r1_fix','Removing all packages which constitute the X Window System ensures users or malicious software cannot start X. To do so, run the following command: 
 ...
 apt-get -y purge xserver-common' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000291','C-46236r1_chk','To ensure the X Windows package group is removed, run the following command: 
...
$ service nfs-kernel-server restart' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000309','C-46239r1_chk','To verify insecure file locking has been disabled, run the following command: 
...
gateway=[local LAN default gateway]' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000292','C-46242r2_chk','To verify that DHCP is not being used, examine the following file for each interface.
...
If no line is returned, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38683','SV-50484r1_rule','default','low','All accounts on the system must have unique user or account names','Unique usernames allow for accountability on the system.','F-43632r1_fix','Change usernames, or delete accounts, so each has a unique name.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000296','C-46245r1_chk','Run the following command to check for duplicate account names: 
...
A documented site-defined number may be substituted for 10 in the above.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000319','C-46246r2_chk','Run the following command to ensure the "maxlogins" value is configured for all users on the system:
...
"[YYYY-MM-DD]" indicates the documented expiration date for the account.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000297','C-46247r1_chk','For every temporary account, run the following command to obtain its account aging and expiration information: 
...
Make sure you configure it to persist after reboot.
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000320','C-46248r1_chk','Check if IPv6 module is enabled or not using the following command :
...
select add_rule_data ('default','V-38687','SV-50488r2_rule','default','low','The system must provide VPN connectivity for communications over untrusted networks.','Providing the ability for remote users or systems to initiate a secure VPN connection protects information when it is transmitted over a wide area network.','F-43636r1_fix','The Openswan package provides an implementation of IPsec and IKE, which permits the creation of secure tunnels over untrusted networks. The "openswan" package can be installed with the following command: 
 ...
 apt-get install openswan' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000321','C-46249r2_chk','If the system does not communicate over untrusted networks, this is not applicable.
...
greeter-show-manual-login=true' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000527','C-49197r4_chk','If the system is NON-GUI based system, this is not applicable
...
To display a banner, this setting must be enabled and then banner text must also be set.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000324','C-46250r3_chk','If the system is NON-GUI based system, this is not applicable.
...
"I"ve read AND consent to terms in IS user agreem"t."' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000326','C-46252r3_chk','If the system is NON-GUI based system, this is not applicable.
...
"[YYYY-MM-DD]" indicates the documented expiration date for the account.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000298','C-46251r1_chk','For every emergency account, run the following command to obtain its account aging and expiration information: 
...
select add_rule_data ('default','V-38691','SV-50492r2_rule','default','medium','The Bluetooth service must be disabled.','Disabling the "bluetooth" service prevents the system from attempting connections to Bluetooth devices, which entails some security risk. Nevertheless, variation in this risk decision may be expected due to the utility of Bluetooth connectivity and its limited range.','F-43640r1_fix','The "bluetooth" service can be disabled with the following command: 
 ...
 service bluetooth stop' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000331','C-46253r3_chk','Run the following command to verify "bluetooth" is disabled through current runtime configuration:
...
A value of 35 is recommended. If a password is currently on the verge of expiration, then 35 days remain until the account is automatically disabled. However, if the password will not expire for another 60 days, then 95 days could elapse until the account would be automatically disabled. See the "useradd" man page for more information. Determining the inactivity timeout must be done with careful consideration of the length of a "normal" period of inactivity for users in the particular environment. Setting the timeout too low incurs support costs and also has the potential to impact availability of the system to legitimate users.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000334','C-46254r2_chk','To verify the "INACTIVE" setting, run the following command:
...
password required pam_cracklib.so maxrepeat=3 ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000299','C-46255r1_chk','To check the maximum value for consecutive repeating characters, run the following command: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000302','C-46257r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000303','C-46258r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
To set the sticky bit on a world-writable directory [], run the following command: 
 ...
 chmod +t [DIR]' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000336','C-46259r4_chk','To find world-writable directories that lack the sticky bit, run the following command for each local partition []: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000304','C-46261r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
$ service tftpd-hpa reload
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000338','C-46263r1_chk','Verify "tftp" is configured by inetd super-server daemon with the "-s" option by running the following command:
...
log_ftp_protocol=YES' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000339','C-46264r1_chk','Find if logging is applied to the ftp daemon. 
...
Another keyboard sequence may be substituted for "<Control><Alt>l", which is the default for the Gnome desktop.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000508','C-46030r2_chk','If the system is NON-GUI based system, this is not applicable.
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000198','C-46125r7_chk','If auditd service is not installed or auditd service not running, then rule get failed.
...
select add_rule_data ('default','V-38443','SV-50243r1_rule','default','medium','The /etc/gshadow file must be owned by root.','The "/etc/gshadow" file contains group password hashes. Protection of this file is critical for system security.','F-43388r1_fix','Run the below command to change the ownership of "/etc/gshadow" to root:
 ...
 chown root /etc/gshadow' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000036','C-45998r1_chk','To check the ownership of "/etc/gshadow", run the command: 
...
select add_rule_data ('default','V-38445','SV-50245r2_rule','default','medium','Audit log files must be group-owned by root.','If non-privileged users can write to audit logs, audit trails can be modified or destroyed.','F-43390r1_fix','Change the group owner of the audit log files with the following command: 
 ...
 chgrp root []' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000522','C-46000r1_chk','Run the following command to check the group owner of the system audit log file(s): 
...
select add_rule_data ('default','V-38448','SV-50248r1_rule','default','medium','The /etc/gshadow file must be group-owned by root.','The "/etc/gshadow" file contains group password hashes. Protection of this file is critical for system security.','F-43393r1_fix','To change the group owner of "/etc/gshadow" to root, run the command: 
 ...
 chgrp root /etc/gshadow' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000037','C-46003r1_chk','To check the group ownership of "/etc/gshadow", run the command: 
...
select add_rule_data ('default','V-38449','SV-50249r1_rule','default','medium','The /etc/gshadow file must have mode 0000.','The /etc/gshadow file contains group password hashes. Protection of this file is critical for system security.','F-43394r1_fix','To properly set the permissions of "/etc/gshadow", run the command: 
 ...
 chmod 0000 /etc/gshadow' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000038','C-46004r1_chk','To check the permissions of "/etc/gshadow", run the command: 
...
select add_rule_data ('default','V-38450','SV-50250r1_rule','default','medium','The /etc/passwd file must be owned by root.','The "/etc/passwd" file contains information about the users that are configured on the system. Protection of this file is critical for system security.','F-43395r1_fix','Run the below command to change the ownership of "/etc/passwd" to root:  
 ...
 chown root /etc/passwd' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000039','C-46005r1_chk','To check the ownership of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38451','SV-50251r1_rule','default','medium','The /etc/passwd file must be group-owned by root.','The "/etc/passwd" file contains information about the users that are configured on the system. Protection of this file is critical for system security.','F-43396r1_fix','To change the group owner of "/etc/passwd" to root, run the command: 
 ...
 chgrp root /etc/passwd' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000040','C-46006r1_chk','To check the group ownership of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38457','SV-50257r1_rule','default','medium','The /etc/passwd file must have mode 0644 or less permissive.','If the "/etc/passwd" file is writable by a group-owner or the world the risk of its compromise is increased. The file contains the list of accounts on the system and associated information, and protection of this file is critical for system security.','F-43397r1_fix','To properly set the permissions of "/etc/passwd", run the command: 
 ...
 chmod 0644 /etc/passwd' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000041','C-46007r1_chk','To check the permissions of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38458','SV-50258r1_rule','default','medium','The /etc/group file must be owned by root.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43403r1_fix','Run the below command to change the ownership of "/etc/group" to root:
 ...
 chown root /etc/group' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000042','C-46013r1_chk','To check the ownership of "/etc/group", run the command: 
...
select add_rule_data ('default','V-38459','SV-50259r1_rule','default','medium','The /etc/group file must be group-owned by root.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43404r1_fix','To change the group owner of "/etc/group" to root, run the command: 
 ...
 chgrp root /etc/group' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000043','C-46014r1_chk','To check the group ownership of "/etc/group", run the command: 
...
select add_rule_data ('default','V-38461','SV-50261r1_rule','default','medium','The /etc/group file must have mode 0644 or less permissive.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43406r1_fix','To properly set the permissions of "/etc/group", run the command: 
 ...
 chmod 644 /etc/group' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000044','C-46015r1_chk','To check the permissions of "/etc/group", run the command: 
...
If any file in these directories is found to be group-writable or world-writable, correct its permission with the following command: 
 ...
 chmod go-w []' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000045','C-46019r4_chk','System-wide shared library files, which are linked to executables during process load time or run time, are stored in the following directories by default: 
...
If any file in these directories is found to be owned by a user other than root, correct its ownership with the following command: 
 ...
 chown root []' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000046','C-46021r1_chk','System-wide shared library files, which are linked to executables during process load time or run time, are stored in the following directories by default: 
...
If any file in these directories is found to be group-writable or world-writable, change its permission with the following command: 
 ...
 chmod go-w []' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000047','C-46024r3_chk','System executables are stored in the following directories by default: 
...
If any file [] in these directories is owned by non-root user, change its ownership with the following command:
 ...
 chown root [FILE]' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000048','C-46027r1_chk','System executables are stored in the following directories by default: 
...
select add_rule_data ('default','V-38493','SV-50294r1_rule','default','medium','Audit log directories must have mode 0755 or less permissive.','If users can delete audit logs, audit trails can be modified or destroyed.','F-43440r1_fix','Change the mode of the audit log directories with the following command: 
 ...
 chmod go-w []
If auditd package is not installed then rule will install it before fixing changes.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000385','C-46050r1_chk','Run the following command to check the mode of the system audit directories: 
...
select add_rule_data ('default','V-38495','SV-50296r1_rule','default','medium','Audit log files must be owned by root.','If non-privileged users can write to audit logs, audit trails can be modified or destroyed.','F-43443r1_fix','Change the owner of the audit log files with the following command: 
 ...
 chown root []
If auditd package is not installed then rule will install it before fixing changes.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000384','C-46053r1_chk','Run the following command to check the owner of the system audit logs: 
...
select add_rule_data ('default','V-38498','SV-50299r1_rule','default','medium','Audit log files must have mode 0640 or less permissive.','If users can write to audit logs, audit trails can be modified or destroyed.','F-43445r1_fix','Change the mode of the audit log files with the following command: 
 ...
 chmod 0640 []
If auditd package is not installed then rule will install it before fixing changes.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000383','C-46055r1_chk','Run the following command to check the mode of the system audit logs: 
...
select add_rule_data ('default','V-38502','SV-50303r1_rule','default','medium','The /etc/shadow file must be owned by root.','The "/etc/shadow" file contains the list of local system accounts and stores password hashes. Protection of this file is critical for system security. Failure to give ownership of this file to root provides the designated owner with access to sensitive information which could weaken the system security posture.','F-43449r1_fix','Run the below command to change the ownership of "/etc/shadow" to root:
 ...
 chown root /etc/shadow' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000033','C-46059r1_chk','To check the ownership of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38503','SV-50304r1_rule','default','medium','The /etc/shadow file must be group-owned by root.','The "/etc/shadow" file stores password hashes. Protection of this file is critical for system security.','F-43450r1_fix','To change the group owner of "/etc/shadow" to root, run the command: 
 ...
 chgrp root /etc/shadow' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000034','C-46060r1_chk','To check the group ownership of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38504','SV-50305r1_rule','default','medium','The /etc/shadow file must have mode 0000.','The "/etc/shadow" file contains the list of local system accounts and stores password hashes. Protection of this file is critical for system security. Failure to give ownership of this file to root provides the designated owner with access to sensitive information which could weaken the system security posture.','F-43451r1_fix','To properly set the permissions of "/etc/shadow", run the command: 
 ...
 chmod 0000 /etc/shadow' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000035','C-46061r2_chk','To check the permissions of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38579','SV-50380r1_rule','default','medium','The system boot loader configuration file(s) must be owned by root.','Only root should be able to modify important boot parameters.','F-43527r1_fix','The file "/etc/default/grub" should be owned by the "root" user to prevent destruction or modification of the file. To change the owner of "/etc/default/grub" to root, run the command: 
 ...
 chown root /etc/default/grub' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000065','C-46137r1_chk','To check the ownership of "etc/default/grub", run the command:
...
select add_rule_data ('default','V-38581','SV-50382r1_rule','default','medium','The system boot loader configuration file(s) must be group-owned by root.','The "root" group is a highly-privileged group. Furthermore, the group-owner of this file should not have any access privileges anyway.','F-43529r1_fix','The file "/etc/default/grub" should be group-owned by the "root" group to prevent destruction or modification of the file. To change the group owner of "/etc/default/grub" to root, run the command: 
 ...
 chgrp root /etc/default/grub' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000066','C-46139r1_chk','To check the group ownership of "/etc/default/grub", run the command:
$ ls -lL /etc/default/grub
...
Boot partitions based on VFAT, NTFS, or other non-standard configurations may require alternative measures.' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000067','C-46141r2_chk','To check the permissions of /boot/grub/grub.cfg, run the command:
...
If the permissions are not 600 or more restrictive, run the following command to correct this:
 ...
 chmod 0600 [LOGFILE]' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000135','C-46181r2_chk','All rsyslog-generated log files must have mode 0600 or less permissive. These log files are determined by the second part of each Rule line in "/etc/rsyslog.conf" and typically all appear in "/var/log". For each log file [] referenced in "/etc/rsyslog.conf", run the following command to inspect the file"s permissions: 
...
select add_rule_data ('default','V-51363','SV-65573r1_rule','default','medium','The system must use a Linux Security Module configured to enforce limits on system services.','Setting the SELinux state to enforcing ensures SELinux is able to confine potentially compromised processes to the security policy, which is designed to prevent them from causing damage to the system or further elevating their privileges. ','F-56165r1_fix','Change the profile modules to enforcing by run the following command:
 ...
 aa-enforce /etc/apparmor.d/*
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000020','C-53703r1_chk','Run the following command to check the security module is set with “enforcing�? 
...
session required pam_lastlog.so showfailed' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000372','C-54013r1_chk','To ensure that last logon/access notification is configured correctly, run the following command:
...
[]
   ' , '[debian-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000528','C-58279r1_chk','
