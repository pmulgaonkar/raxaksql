select add_profile_data ('[rpm-based Linux] DISA - Mission Critical Classified (Full)' ,'Profile Customized by CloudRaxak' ,'Linux-based OS','' ,'2015-05-26' ,'Profile by Cloud Raxak') from dual;
...
Stop the service if it is already running: 
 ...
 service autofs stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000526','C-45991r1_chk','To verify the "autofs" service is not running:
...
and run the following command to update the grub.
 ...
 grub2-mkconfig -o /boot/grub2/grub.cfg
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000098','C-46104r2_chk','Check if IPv6 module is enabled or not using the following command :
...
To verify if IPv6 is disabled in GRUB, run the following:
 ...
 grep -iw "ipv6.disable" /boot/grub2/grub.cfg | grep -v "^\s*"
...
Make sure you added rule to accept connections from CloudRaxak by using following command.
 ...
 ip6tables -A INPUT -m string --hex-string "|0A|cloudraxak|03|net" --algo bm -j ACCEPT
...
Make sure all your iptables configuration is persistent.
            ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000523','C-45999r2_chk','Check if IPv6 module is enabled or not using the following command :
...
select add_rule_data ('default','V-38669','SV-50470r1_rule','default','low','The postfix service must be enabled for mail delivery.','Local mail delivery is essential to some system maintenance and notification tasks.','F-43618r1_fix','The Postfix mail transfer agent is used for local mail delivery within the system. The default configuration only listens for connections to the default SMTP port (port 25) on the loopback interface (127.0.0.1). It is recommended to leave this service enabled for local mail delivery. The "postfix" service can be enabled with the following command: 
 ...
 service postfix start' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000287','C-46230r1_chk','Run the following command to determine the current status of the "postfix" service:
...
select add_rule_data ('default','V-38446','SV-50246r1_rule','default','medium','The mail system must forward all mail for root to one or more system administrators.','A number of system services utilize email messages sent to the root user to notify system administrators of active or impending issues.  These messages must be forwarded to at least one monitored email address.','F-43391r1_fix','Set up an alias for root that forwards to a monitored email address:
 ...
 newaliases' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000521','C-46001r1_chk','Find the list of alias maps used by the Postfix mail server:
...
inet_interfaces = localhost' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000249','C-46182r2_chk','If the system is an authorized mail relay host, this is not applicable. 
...
yum reinstall [affected_package]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000519','C-46002r3_chk','The following command will list which files on the system have file hashes different from what is expected by the RPM database. 

 rpm -Va | awk "$1 ~ /..5/ ANDAND $2 != "c""


If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38452','SV-50252r1_rule','default','low','The system package management tool must verify permissions on all files and directories associated with packages.','Permissions on system binaries and configuration files that are too generous could allow an unauthorized user to gain privileges that they should not have. The permissions set by the vendor should be maintained. Any deviations from this baseline should be investigated.','F-43398r1_fix','The RPM package management system can restore file access permissions of package files and directories. The following command will update permissions on files and directories with permissions different from what is expected by the RPM database: 

 rpm --setperms []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000518','C-46008r1_chk','The following command will list which files and directories on the system have permissions different from what is expected by the RPM database: 
...
If the existing permissions are more permissive than those expected by RPM, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38453','SV-50253r1_rule','default','low','The system package management tool must verify group-ownership on all files and directories associated with packages.','Group-ownership of system binaries and configuration files that is incorrect could allow an unauthorized user to gain privileges that they should not have. The group-ownership set by the vendor should be maintained. Any deviations from this baseline should be investigated.','F-43399r1_fix','The RPM package management system can restore group-ownership of the package files and directories. The following command will update files and directories with group-ownership different from what is expected by the RPM database: 

 rpm -qf []
 rpm --setugids [package]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000517','C-46009r1_chk','The following command will list which files on the system have group-ownership different from what is expected by the RPM database: 

 rpm -Va | grep "^......G"


If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38454','SV-50254r1_rule','default','low','The system package management tool must verify ownership on all files and directories associated with packages.','Ownership of system binaries and configuration files that is incorrect could allow an unauthorized user to gain privileges that they should not have. The ownership set by the vendor should be maintained. Any deviations from this baseline should be investigated.','F-43400r1_fix','The RPM package management system can restore ownership of package files and directories. The following command will update files and directories with ownership different from what is expected by the RPM database: 

 rpm -qf []
 rpm --setugids [package]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000516','C-46010r1_chk','The following command will list which files on the system have ownership different from what is expected by the RPM database: 

 rpm -Va | grep "^.....U"


If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38455','SV-50255r1_rule','default','low','The system must use a separate file system for /tmp.','The "/tmp" partition is used as temporary storage by many programs. Placing "/tmp" in its own partition enables the setting of more restrictive mount options, which can help protect programs which use it.','F-43387r1_fix','The tmp partition is used as temporary storage by many programs. Placing /tmp in its own partition enables the setting of more restrictive mount options, which can help to protect the programs that use it. Please create a separate file system for /tmp.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000001','C-45997r1_chk','Run the following command to determine if "/tmp" is on its own partition or logical volume: 
...
select add_rule_data ('default','V-38460','SV-50260r1_rule','default','low','The NFS server must not have the all_squash option enabled.','The "all_squash" option maps all client requests to a single anonymous uid/gid on the NFS server, negating the ability to track file access by user ID.','F-43405r1_fix','Remove any instances of the "all_squash" option from the file "/etc/exports".  Restart the NFS daemon for the changes to take effect.
 ...
 service nfs restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000515','C-46016r1_chk','If the NFS server is read-only, in support of unrestricted access to organizational content, this is not applicable.
...
INFO: minlen should be >= 14 and the credits limit should be less or equal to 0 for the validation criteria in the rule.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000050','C-46029r1_chk','To check the minimum password length and to check the other settings required for the password authentication, run the command:
...
select add_rule_data ('default','V-38476','SV-50276r3_rule','default','high','Vendor-provided cryptographic certificates must be installed to verify the integrity of system software.','The Red Hat GPG keys are necessary to cryptographically verify packages are from Red Hat. ','F-43421r3_fix','To ensure the system can cryptographically verify base software packages come from Red Hat (and to connect to the Red Hat Network to receive them), the Red Hat GPG keys must be installed properly. To install the Red Hat GPG keys, run:
 ...
 rpm --import /etc/pki/rpm-gpg/*' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000008','C-46031r3_chk','To ensure that the GPG keys are installed, run:
...
select add_rule_data ('default','V-38478','SV-50278r2_rule','default','low','The Red Hat Network Service (rhnsd) service must not be running, unless using RHN or an RHN Satellite.','Although systems management and patching is extremely important to system security, management by a system outside the enterprise enclave is not desirable for some environments. However, if the system is being managed by RHN or RHN Satellite Server the "rhnsd" daemon can remain on.','F-43423r2_fix','The Red Hat Network service automatically queries Red Hat Network servers to determine whether there are any actions that should be executed, such as package updates. This only occurs if the system was registered to an RHN server or satellite and managed as such. The "rhnsd" service can be disabled with the following commands: 
 ...
 service rhnsd stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000009','C-46033r2_chk','Run the following command to verify "rhnsd" is disabled through current runtime configuration: 
...
$ pwunconv; pwconv' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000053','C-46034r1_chk','To check the maximum password age, run the command: 
...
$ pwunconv; pwconv' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000051','C-46032r1_chk','To check the minimum password age, run the command: 
...
Add "dcredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of a digit in password.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000056','C-46037r1_chk','To check how many digits are required in a password, run the following command: 
...
If the "yum" system package management tool is not used to update the system, verify with the SA that installed packages are cryptographically signed.','Customized rule by Cloud Raxak') from dual;
...
$ service sshd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000507','C-46041r2_chk','Verify the value associated with the "PrintLastLog" keyword in /etc/ssh/sshd_config:
...
At console or tty logon, a user must be presented with the last successful login date and time.  The "hushlogin" file determines whether this occurs.','F-43432r1_fix','Remove any "hushlogin" files from the system:
 ...
 rm /etc/hushlogins
 rm ~<userid>/.hushlogin' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000506','C-46042r1_chk','Verify there are no "hushlogin" files active on the system:
 ls -l /etc/hushlogins
...
Implement a process whereby OS data is backed up from the system in accordance with local policies.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000505','C-46044r1_chk','Ask an administrator if a process exists to back up OS data from the system, including configuration data. 

If such a process does not exist, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38487','SV-50288r1_rule','default','low','The system package management tool must cryptographically verify the authenticity of all software packages during installation.','Ensuring all packages" cryptographic signatures are valid prior to installation ensures the provenance of the software and protects against malicious tampering.','F-43433r1_fix','To ensure signature checking is not disabled for any repos, update any lines from files in "/etc/yum.repos.d" of the form: 

gpgcheck=1' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000015','C-46043r1_chk','To determine whether "yum" has been configured to disable "gpgcheck" for any repos, inspect all files in "/etc/yum.repos.d" and ensure the following does not appear in any sections: 
...
Implement a process whereby user data is backed up from the system in accordance with local policies.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000504','C-46045r1_chk','Ask an administrator if a process exists to back up user data from the system. 
...
select add_rule_data ('default','V-38489','SV-50290r1_rule','default','medium','A file integrity tool aide must be installed.','The AIDE package must be installed if it is to be available for integrity checking.','F-43436r1_fix','Install the AIDE package with the command: 
 ...
 yum install aide' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000016','C-46046r1_chk','Run the following command to determine if the "aide" package is installed: 
...
If this check produces any unexpected output, investigate. ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000018','C-53727r1_chk','To find the location of the AIDE database file, run the following command:
...
' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000019','C-46048r1_chk','The existence of the file named "hosts.equiv" or ".rhosts" inside a user home directory indicates the presence of an Rsh trust relationship, use the command:
...
Note:  Virtual console entries are not limited to those listed above.  Any lines starting with "vc/" followed by numerals should be removed.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000027','C-46049r1_chk','To check for virtual console entries which permit root login, run the following command: 
...
Note:  Serial port entries are not limited to those listed above.  Any lines starting with "ttyS" followed by numerals should be removed' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000028','C-46051r1_chk','To check for serial port entries which permit root login, run the following command: 
...
Disable logon access to these accounts with the command: 
 ...
 passwd -l []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000029','C-46052r2_chk','To obtain the system accounts which is unlocked run the following command: 
...
select add_rule_data ('default','V-38497','SV-50298r2_rule','default','high','The system must not have accounts configured with blank or null passwords.','If an account has an empty password, anyone could log in and run commands with the privileges of that account. Accounts with empty passwords should never be used in operational environments.','F-43444r4_fix','If an account is configured for password authentication but does not have an assigned password, it may be possible to log onto the account without authentication. Remove any instances of the "nullok" option in "/etc/pam.d/system-auth" to prevent logons with empty passwords.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000030','C-46054r2_chk','To verify that null passwords cannot be used, run the following command: 

 grep -w nullok /etc/pam.d/system-auth | grep -v "^"
...
Note that any updates made to "/etc/pam.d/system-auth" and "/etc/pam.d/password-auth" may be overwritten by the "authconfig" program.  The "authconfig" program should not be used.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000061','C-46131r4_chk','To ensure the failed password attempt policy is configured correctly, run the following command: 
...
If that is not the case, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38501','SV-50302r4_rule','default','medium','The system must disable accounts after excessive login failures within a 15-minute interval.','Locking out user accounts after a number of incorrect attempts within a specific period of time prevents direct password guessing attacks.','F-43448r6_fix','Utilizing "pam_faillock.so", the "fail_interval" directive configures the system to lock out accounts after a number of incorrect logon attempts. Modify the content of both "/etc/pam.d/system-auth" and "/etc/pam.d/password-auth" as follows: 
...
Note that any updates made to "/etc/pam.d/system-auth" and "/etc/pam.d/password-auth" may be overwritten by the "authconfig" program.  The "authconfig" program should not be used.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000357','C-46058r2_chk','To ensure the failed password attempt policy is configured correctly, run the following command:

$ grep -w pam_faillock /etc/pam.d/system-auth /etc/pam.d/password-auth | grep -v "^"

For each file, the output should show "fail_interval=<interval-in-seconds>" where "interval-in-seconds" is 900 (15 minutes) or greater. If the "fail_interval" parameter is not set, the default setting of 900 seconds is acceptable. If that is not the case, this is a finding. ','Customized rule by Cloud Raxak') from dual;
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000082','C-46068r3_chk','Check if IP forwarding for IPv4 is enabled on the system, by querying the following command:
...
select add_rule_data ('default','V-38512','SV-50313r2_rule','default','medium','The operating system must prevent public IPv4 access into an organizations internal networks, except as appropriately mediated by managed interfaces employing boundary protection devices.','The "iptables" service provides the system"s host-based firewalling capability for IPv4 and ICMP.','F-43459r2_fix','Note: Following steps may block you from accessing internet. Hence, make sure you run the commands cautiously.
...
or add net.ipv4.conf.all.forwarding=1 to following file

 /etc/sysctl.conf

To set the default FORWARD policy to DROP run following commands for IPv4 Firewall.

 iptables -P FORWARD DROP

Make sure you configure the rules to persist after reboot.
' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000117','C-46069r2_chk','Run the following command to determine the current status of the Firewall:
...
Make sure ip_Forwarding is enabled.
 ...
 sysctl -a 2>/dev/null | grep "net.ipv4" | grep "forwarding = 1"
...
If ip forwarding is enabled and the default FORWARD policy is not set to DROP, this is a finding.
...
Make sure all your iptables configuration is persistent.
            ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000120','C-46070r1_chk','Run the following command to determine the current status of the Firewall:
...
$ install tipc /bin/true' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000127','C-46074r3_chk','If the system is configured to prevent the loading of the "tipc" kernel module, it will contain lines inside any file in "/etc/modprobe.d" or the deprecated "/etc/modprobe.conf". These lines instruct the module loading system to run another program (such as "/bin/true") upon a module "install" event. Run the following command to search for such lines in all files in "/etc/modprobe.d" and the deprecated "/etc/modprobe.conf": 
...
*.* :omrelp:[loghost.example.com]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000136','C-46078r1_chk','To ensure logs are sent to a remote host, examine the file "/etc/rsyslog.conf". If using UDP, a line similar to the following should be present: 
...
*.* :omrelp:[loghost.example.com]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000137','C-46269r1_chk','To ensure logs are sent to a remote host, examine the file "/etc/rsyslog.conf". If using UDP, a line similar to the following should be present: 
...
Now, to get recently added audit rules into effect run the following command: 
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000167','C-46080r2_chk','To determine if the system is configured to audit calls to the "settimeofday" system call, run the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000083','C-46081r3_chk','Check if system is configured to reject IPv4 source-routed packets on any interface, by querying the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000084','C-46082r2_chk','Check if the system is configured to reject ICMPv4 redirect packets, by querying the following command:
...
Now, to get recently added audit rules into effect run the following command: 
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000169','C-46083r3_chk','If the system is 64-bit only, this is not applicable.
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000086','C-46084r2_chk','To check if system accepts ICMPv4 secure redirect packets on any interface, run the following command:
...
Now, to get recently added audit rules into effect run the following command: 
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000171','C-46085r2_chk','To determine if the system is configured to audit calls to the "clock_settime" system call, run the following command:
...
or restart sysctl' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000088','C-46086r3_chk','Check if the system is configured to log Martian packets, by querying the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000089','C-46088r2_chk','Check if system is configured to reject IPv4 source-routed packets by default, by running the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000174','C-46090r1_chk','To determine if the system is configured to audit account changes, run the following command: 
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000090','C-46089r2_chk','Check if the system is configured to reject ICMPv4 secure redirect packets by default, by querying the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000091','C-46091r2_chk','Check if the system is configured to ignore ICMPv4 redirect messages by default, by querying the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000175','C-46092r1_chk','To determine if the system is configured to audit account changes, run the following command: 
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000092','C-46093r2_chk','The status of the "net.ipv4.icmp_echo_ignore_broadcasts" kernel parameter can be queried by running the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000176','C-46094r1_chk','To determine if the system is configured to audit account changes, run the following command: 
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000093','C-46095r2_chk','Check if the system is configured to ignore ICMPv4 bogus error responses, by querying the following command: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000177','C-46096r1_chk','To determine if the system is configured to audit account changes, run the following command: 
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000095','C-46097r2_chk','Check if the system is configured to use TCP syncookies when experiencing a TCP SYN flood, by querying the following command:
...
select add_rule_data ('default','V-38540','SV-50341r2_rule','default','low','The audit system must be configured to audit modifications to the systems network configuration.','The network environment should not be modified by anything other than administrator action. Any change to network parameters should be audited.','F-43488r2_fix','Add the following to "/etc/audit/audit.rules", setting ARCH to either b32 or b64 as appropriate for your system: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000182','C-46098r1_chk','To determine if the system is configured to audit changes to its network configuration, run the following command: 
...
select add_rule_data ('default','V-38541','SV-50342r1_rule','default','low','The audit system must be configured to audit modifications to the systems Mandatory Access Control (MAC) configuration (SELinux).','The system"s mandatory access policy (SELinux) should not be arbitrarily changed by anything other than administrator action. All changes to MAC policy should be audited.','F-43489r1_fix','Add the following to "/etc/audit/audit.rules": 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000183','C-46099r1_chk','To determine if the system is configured to audit changes to its SELinux configuration files, run the following command: 

 grep -w "/etc/selinux" /etc/audit/audit.rules
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000096','C-46100r2_chk','Check if system is configured to use the reverse-path filter for IPv4 network traffic on all interfaces enabled
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000184','C-46101r2_chk','To determine if the system is configured to audit calls to the "chmod" system call, run the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000097','C-46102r2_chk','Check if the system is configured to use TCP syncookies when experiencing a TCP SYN flood, by querying the following command:                       
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000185','C-46103r2_chk','To determine if the system is configured to audit calls to the "chown" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000186','C-46105r2_chk','To determine if the system is configured to audit calls to the "fchmod" system call, run the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000099','C-46106r3_chk','Check if IPv6 module is enabled or not using the following command :
...
and configure the same to deny-all packets except coming from CloudRaxak.
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000103','C-46107r3_chk','Check if IPv6 module is enabled or not using the following command :
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000187','C-46108r2_chk','To determine if the system is configured to audit calls to the "fchmodat" system call, run the following command:
...
If no line is returned, this is a finding. ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38551','SV-50352r3_rule','default','medium','The operating system must connect to external networks or information systems only through managed IPv6 interfaces consisting of boundary protection devices arranged in accordance with an organizational security architecture.','The "ip6tables" service provides the system"s host-based firewalling capability for IPv6 and ICMPv6.','F-43499r2_fix','Note: Following steps may block you from accessing internet. Hence, make sure you run the commands cautiously.
...
To set the default OUTPUT policy to DROP run following commands for IPv6 Firewall.

 ip6tables -P OUTPUT DROP

Make sure you configure the rules to persist after reboot.
                        ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000106','C-46109r3_chk','Check if IPv6 module is enabled or not using the following command :
...
Make sure default OUTPUT policy is set to DROP.

 ip6tables -L  OUTPUT | head -1

If the default OUTPUT policy is not set to DROP, this is a finding.

Make sure default OUTPUT policy is set to DROP.

Also make sure your ip6tables configuration is persistent.
                        ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38552','SV-50353r3_rule','default','low','The audit system must be configured to audit all discretionary access control permission modifications using fchown.','The changing of file permissions could indicate that a user is attempting to gain access to information that would otherwise be disallowed. Auditing DAC modifications can facilitate the identification of patterns of abuse among both authorized and unauthorized users.','F-43500r2_fix','At a minimum, the audit system should collect file permission changes for all users and root. Add the following to "/etc/audit/audit.rules": 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000188','C-46110r2_chk','To determine if the system is configured to audit calls to the "fchown" system call, run the following command:
...
If no line is returned, this is a finding. ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38553','SV-50354r3_rule','default','medium','The operating system must prevent public IPv6 access into an organizations internal networks, except as appropriately mediated by managed interfaces employing boundary protection devices.','The "ip6tables" service provides the system"s host-based firewalling capability for IPv6 and ICMPv6.','F-43501r2_fix','Note: Following steps may block you from accessing internet. Hence, make sure you run the commands cautiously.
...
or add net.ipv6.conf.all.forwarding=1 to following file

 /etc/sysctl.conf

To set the default FORWARD policy to DROP run following commands for IPv6 Firewall.

 ip6tables -P FORWARD DROP

Make sure you configure the rules to persist after reboot.
                        ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000107','C-46111r3_chk','Check if IPv6 module is enabled or not using the following command :
...
make sure ip_Forwarding is enabled.
 ...
 sysctl -a 2>/dev/null | grep "net.ipv6" | grep "forwarding = 1"
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000189','C-46112r2_chk','To determine if the system is configured to audit calls to the "fchownat" system call, run the following command:
...
and configure the same to deny-all packets except coming from CloudRaxak.
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000113','C-46113r2_chk','Run the following command to determine the current status of the Firewall:

 iptables -L

If above command gives error, this is a finding. 
                ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38556','SV-50357r3_rule','default','low','The audit system must be configured to audit all discretionary access control permission modifications using fremovexattr.','The changing of file permissions could indicate that a user is attempting to gain access to information that would otherwise be disallowed. Auditing DAC modifications can facilitate the identification of patterns of abuse among both authorized and unauthorized users.','F-43504r2_fix','At a minimum, the audit system should collect file permission changes for all users and root. Add the following to "/etc/audit/audit.rules": 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000190','C-46114r2_chk','To determine if the system is configured to audit calls to the "fremovexattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000191','C-46115r2_chk','To determine if the system is configured to audit calls to the "fsetxattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000192','C-46116r2_chk','To determine if the system is configured to audit calls to the "lchown" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000193','C-46117r2_chk','To determine if the system is configured to audit calls to the "lremovexattr" system call, run the following command:
...
If no line is returned, this is a finding. ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38560','SV-50361r2_rule','default','medium','The operating system must connect to external networks or information systems only through managed IPv4 interfaces consisting of boundary protection devices arranged in accordance with an organizational security architecture.','The "iptables" service provides the system"s host-based firewalling capability for IPv4 and ICMP.','F-43508r2_fix','Note: Following steps may block you from accessing internet. Hence, make sure you run the commands cautiously.
...
To set the default OUTPUT policy to DROP run following commands for IPv4 Firewall.

 iptables -P OUTPUT DROP

Make sure you configure the rules to persist after reboot.
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000116','C-46118r2_chk','Run the following command to determine the current status of the Firewall:
...
Make sure you have added rule to connect to CloudRaxak by using following command.

 iptables -A OUTPUT -m string --hex-string "|0A|cloudraxak|03|net" --algo bm -j ACCEPT

Also make sure your iptables configuration is persistent.
   ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38561','SV-50362r3_rule','default','low','The audit system must be configured to audit all discretionary access control permission modifications using lsetxattr.','The changing of file permissions could indicate that a user is attempting to gain access to information that would otherwise be disallowed. Auditing DAC modifications can facilitate the identification of patterns of abuse among both authorized and unauthorized users.','F-43509r2_fix','At a minimum, the audit system should collect file permission changes for all users and root. Add the following to "/etc/audit/audit.rules": 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000194','C-46119r2_chk','To determine if the system is configured to audit calls to the "lsetxattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000195','C-46121r2_chk','To determine if the system is configured to audit calls to the "removexattr" system call, run the following command:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000196','C-46123r2_chk','To determine if the system is configured to audit calls to the "setxattr" system call, run the following command:
...
If no line is returned, this is a finding. ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38566','SV-50367r2_rule','default','low','The audit system must be configured to audit failed attempts to access files and programs.','Unsuccessful attempts to access files could be an indicator of malicious activity on a system. Auditing these events could serve as evidence of potential system compromise.','F-43514r2_fix','At a minimum, the audit system should collect unauthorized file accesses for all users and root. Add the following to "/etc/audit/audit.rules", setting ARCH to either b32 or b64 as appropriate for your system: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000197','C-46124r1_chk','To verify that the audit system collects unauthorized file accesses, run the following commands: 
...
select add_rule_data ('default','V-38568','SV-50369r3_rule','default','low','The audit system must be configured to audit successful file system mounts.','The unauthorized exportation of data to external media could result in an information leak where classified information, Privacy Act information, and intellectual property could be lost. An audit trail should be created each time a filesystem is mounted to help identify and guard against information loss.','F-43516r2_fix','At a minimum, the audit system should collect media exportation events for all users and root. Add the following to "/etc/audit/audit.rules", setting ARCH to either b32 or b64 as appropriate for your system: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000199','C-46126r2_chk','To verify that auditing is configured for all media exportation events, run the following command: 
...
Add "ucredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of an uppercase character in a password.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000057','C-46127r1_chk','To check how many uppercase characters are required in a password, run the following command: 
...
Add "ocredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of a special character in passwords.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000058','C-46128r1_chk','To check how many special characters are required in a password, run the following command: 
...
Add "lcredit=-1" after pam_cracklib.so in /etc/pam.d/system-auth file to require use of a lowercase character in a password.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000059','C-46129r1_chk','To check how many lowercase characters are required in a password, run the following command: 
...
select add_rule_data ('default','V-38572','SV-50373r1_rule','default','low','The system must require at least four characters be changed between the old and new passwords during a password change.','Requiring a minimum number of different characters during password changes ensures that newly changed passwords should not resemble previously compromised ones. Note that passwords which are changed on compromised systems will still be compromised, however.','F-43520r1_fix','The pam_cracklib module"s "difok" parameter controls requirements for usage of different characters during a password change. Add "difok=[]" after pam_cracklib.so to require differing characters when changing passwords, substituting [NUM] appropriately. requirement is greater or equal to 4.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000060','C-46130r1_chk','To check how many characters must differ during a password change, run the following command: 
...
select add_rule_data ('default','V-38574','SV-50375r2_rule','default','medium','The system must use a FIPS 140-2 approved cryptographic hashing algorithm for generating account password hashes (system-auth).','Using a stronger hashing algorithm makes password cracking attacks more difficult.','F-43522r2_fix','In "/etc/pam.d/system-auth", among potentially other files, the "password" section of the files control which PAM modules execute during a password change. Set the "pam_unix.so" module in the "password" section to include the argument "sha512", as shown below: 

password sufficient pam_unix.so sha512 []

This will help ensure when local users change their passwords, hashes for the new passwords will be generated using the SHA-512 algorithm. This is the default.

Note that any updates made to "/etc/pam.d/system-auth" will be overwritten by the "authconfig" program.  The "authconfig" program should not be used.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000062','C-46132r3_chk','Inspect the "password" section of "/etc/pam.d/system-auth", "/etc/pam.d/system-auth-ac", and other files in "/etc/pam.d" and ensure that the "pam_unix.so" module includes the argument "sha512".

$ grep -w password /etc/pam.d/system-auth | grep -w pam_unix.so | grep -v "^"

If it does not, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38575','SV-50376r4_rule','default','low','The audit system must be configured to audit user deletions of files and programs.','Auditing file deletions will create an audit trail for files that are removed from the system. The audit trail could aid in system troubleshooting, as well as detecting malicious processes that attempt to delete log files to conceal their presence.','F-43523r4_fix','At a minimum, the audit system should collect file deletion events for all users and root. Add the following (or equivalent) to "/etc/audit/audit.rules", setting ARCH to either b32 or b64 as appropriate for your system: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000200','C-46133r3_chk','To determine if the system is configured to audit calls to the "rmdir" system call, run the following command:
...
ENCRYPT_METHOD SHA512' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000063','C-46134r1_chk','Inspect "/etc/login.defs" and ensure the following line appears: 
...
crypt_style = sha512' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000064','C-46135r1_chk','Inspect "/etc/libuser.conf" and ensure the following line appears in the "[]" section: 

 grep -w crypt_style /etc/libuser.conf | grep -v "^"
crypt_style = sha512

If it does not, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38578','SV-50379r1_rule','default','low','The audit system must be configured to audit changes to the /etc/sudoers file.','The actions taken by system administrators should be audited to keep a record of what was executed on the system, as well as, for accountability purposes.','F-43526r1_fix','At a minimum, the audit system should collect administrator actions for all users and root. Add the following to "/etc/audit/audit.rules": 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000202','C-46138r3_chk','To determine if the system is configured to audit execution of module management programs, run the following commands:
...
select add_rule_data ('default','V-38582','SV-50383r2_rule','default','medium','The xinetd service must be disabled if no network services utilizing it are enabled.','The xinetd service provides a dedicated listener service for some programs, which is no longer necessary for commonly-used network services. Disabling it ensures that these uncommon services are not running, and also prevents attacks against xinetd itself.','F-43530r2_fix','The "xinetd" service can be disabled with the following commands: 
 ...
 service xinetd stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000203','C-46140r2_chk','If network services are using the xinetd service, this is not applicable.
...
select add_rule_data ('default','V-38584','SV-50385r1_rule','default','low','The xinetd service must be uninstalled if no network services utilizing it are enabled.','Removing the "xinetd" package decreases the risk of the xinetd service"s accidental (or intentional) activation.','F-43532r1_fix','The "xinetd" package can be uninstalled with the following command: 
 ...
 rpm -e xinetd --nodeps' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000204','C-46142r1_chk','If network services are using the xinetd service, this is not applicable.
...
password --encrypted [password-hash]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000068','C-46143r2_chk','To verify the boot loader password has been set and encrypted, run the following command: 
...
Remove the disable single-user mode authentication  parameters "init=/bin/bash", "init=/bin/sh" from grub files if it exists in kernel parameter line.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000069','C-46145r1_chk',' To verify the single-user mode authentication is disabled, check the "init=/bin/bash", "init=/bin/sh" parameters existancy in kernel argument line in grub file.
...
Mitigation:  If the telnet-server package is configured to only allow encrypted sessions, such as with Kerberos or the use of encrypted network tunnels, the risk of exposing sensitive information is mitigated.','F-43535r1_fix','The "telnet-server" package can be uninstalled with the following command: 
 ...
 rpm -e telnet-server --nodeps' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000206','C-46144r1_chk','Run the following command to determine if the "telnet-server" package is installed: 

 rpm -q telnet-server


If the package is installed, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38588','SV-50389r1_rule','default','medium','The system must not permit interactive boot.','Using interactive boot, the console user could disable auditing, firewalls, or other services, weakening system security.','F-43536r1_fix','To disable the ability for users to perform interactive startups, edit the file "/etc/sysconfig/init". Add or correct the line: 

PROMPT=no

The "PROMPT" option allows the console user to perform an interactive system startup, in which it is possible to select the set of services which are started on boot.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000070','C-46146r1_chk','To check whether interactive boot is disabled, run the following command: 

$ grep -w PROMPT /etc/sysconfig/init | grep -v "^"
...
Mitigation:  If an enabled telnet daemon is configured to only allow encrypted sessions, such as with Kerberos or the use of encrypted network tunnels, the risk of exposing sensitive information is mitigated.','F-43537r1_fix','The "telnet" service can be disabled with the following command: 

 chkconfig telnet off' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000211','C-46147r3_chk','To check that the "telnet" service is disabled in system boot configuration, run the following command: 
...
ctrl+a x' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000071','C-46148r1_chk','Run the following command to determine if the "screen" package is installed: 
...
select add_rule_data ('default','V-38591','SV-50392r1_rule','default','high','The rsh-server package must not be installed.','The "rsh-server" package provides several obsolete and insecure network services. Removing it decreases the risk of those services" accidental (or intentional) activation.','F-43539r1_fix','The "rsh-server" package can be uninstalled with the following command: 
 ...
 rpm -e rsh-server --nodeps' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000213','C-46149r1_chk','Run the following command to determine if the "rsh-server" package is installed: 
...
$ service sshd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000073','C-46150r1_chk','To check if the system login banner is compliant, run the following command: 
...
select add_rule_data ('default','V-38594','SV-50395r2_rule','default','high','The rshd service must not be running.','The rsh service uses unencrypted network communications, which means that data from the login session, including passwords and all other information transmitted during the session, can be stolen by eavesdroppers on the network.','F-43542r3_fix','The "rsh" service, which is available with the "rsh-server" package and runs as a service through xinetd, should be disabled. The "rsh" service can be disabled with the following command: 
 ...
 chkconfig rsh off' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000214','C-46152r2_chk','To check that the "rsh" service is disabled in system boot configuration, run the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000078','C-46153r2_chk','Check if the system is configured to implement virtual address space randomization, by querying the following command:
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000079','C-46155r3_chk','Check if the system is configured to limit the ability of processes to have simultaneous write and execute 
...
select add_rule_data ('default','V-38598','SV-50399r2_rule','default','high','The rexecd service must not be running.','The rexec service uses unencrypted network communications, which means that data from the login session, including passwords and all other information transmitted during the session, can be stolen by eavesdroppers on the network.','F-43546r3_fix','The "rexec" service, which is available with the "rsh-server" package and runs as a service through xinetd, should be disabled. The "rexec" service can be disabled with the following command: 

 chkconfig rexec off' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000216','C-46156r3_chk','To check that the "rexec" service is disabled in system boot configuration, run the following command:
...
Restart the vsftpd daemon.
 ...
 service vsftpd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000348','C-46174r1_chk','If vstfpd service is not installed on the system, this rule is not applicable.
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000080','C-46157r2_chk','Check if the system is configured to ignore ICMPv4 redirects by default, by querying the following command:    
...
or restart sysctl.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000081','C-46159r2_chk','Check if system sends ICMPv4 redirects from any interface, by querying the following command:
...
If is not set to "0" in "/etc/sysctl.conf", this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38602','SV-50403r2_rule','default','high','The rlogind service must not be running.','The rlogin service uses unencrypted network communications, which means that data from the login session, including passwords and all other information transmitted during the session, can be stolen by eavesdroppers on the network.','F-43549r3_fix','The "rlogin" service, which is available with the "rsh-server" package and runs as a service through xinetd, should be disabled. The "rlogin" service can be disabled with the following command: 

 chkconfig rlogin off' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000218','C-46158r3_chk',' 
...
select add_rule_data ('default','V-38603','SV-50404r1_rule','default','medium','The ypserv package must not be installed.','Removing the "ypserv" package decreases the risk of the accidental (or intentional) activation of NIS or NIS+ services.','F-43551r1_fix','The "ypserv" package can be uninstalled with the following command: 
 ...
 rpm -e ypserv --nodeps' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000220','C-46161r1_chk','Run the following command to determine if the "ypserv" package is installed: 
...
select add_rule_data ('default','V-38604','SV-50405r2_rule','default','medium','The ypbind service must not be running.','Disabling the "ypbind" service ensures the system is not acting as a client in a NIS or NIS+ domain.','F-43552r2_fix','The "ypbind" service, which allows the system to act as a client in a NIS or NIS+ domain, should be disabled. The "ypbind" service can be disabled with the following commands: 
 ...
 service ypbind stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000221','C-46162r2_chk','Run the following command to verify "ypbind" is disabled through current runtime configuration: 
...
select add_rule_data ('default','V-38605','SV-50406r2_rule','default','medium','The cron service must be running.','Due to its usage for maintenance and security-supporting tasks, enabling the cron daemon is essential.','F-43553r2_fix','The "crond" service is used to execute commands at preconfigured times. It is required by almost all systems to perform necessary maintenance tasks, such as notifying root of system activity. The "crond" service can be enabled with the following commands: 
 ...
 chkconfig crond on
 service crond start' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000224','C-46163r1_chk','Run the following command to determine the current status of the "crond" service: 
...
select add_rule_data ('default','V-38606','SV-50407r2_rule','default','medium','The tftp-server package must not be installed unless required.','Removing the "tftp-server" package decreases the risk of the accidental (or intentional) activation of tftp services.','F-43554r1_fix','The "tftp-server" package can be removed with the following command: 
 ...
 rpm -e tftp-server --nodeps' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000222','C-46164r1_chk','Run the following command to determine if the "tftp-server" package is installed: 
...
$ service sshd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000227','C-46165r1_chk','To check which SSH protocol version is allowed, run the following command: 
...
$ service sshd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000231','C-46168r1_chk','To ensure the SSH idle timeout will occur when the "ClientAliveCountMax" is set, run the following command: 
...
$ service sshd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000230','C-46167r1_chk','Run the following command to see what the timeout interval is: 
...
select add_rule_data ('default','V-38609','SV-50410r2_rule','default','medium','The TFTP service must not be running.','Disabling the "tftp" service ensures the system is not acting as a tftp server, which does not provide encryption or authentication.','F-43557r4_fix','The "tftp" service should be disabled. The "tftp" service can be disabled with the following command: 
 ...
 chkconfig tftp off' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000223','C-46166r2_chk','To check that the "tftp" service is disabled in system boot configuration, run the following command:
...
IgnoreRhosts yes' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000234','C-46169r1_chk','To determine how the SSH daemon"s "IgnoreRhosts" option is set, run the following command: 
...
' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000236','C-46170r1_chk','To determine how the SSH daemon"s "HostbasedAuthentication" option is set, run the following command: 
...
4. User account password  must be set or RSA key­pairs should be configured.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000237','C-46171r1_chk','To determine how the SSH daemon"s "PermitRootLogin" option is set, run the following command: 
...
$ service ssh restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000239','C-46172r1_chk','To determine how the SSH daemon"s "PermitEmptyPasswords" option is set, run the following command: 
...
$ service sshd restart
                        ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000240','C-46173r1_chk','To determine how the SSH daemon"s "Banner" option is set, run the following command: 
...
$ service sshd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000241','C-46175r1_chk','To ensure users are not able to present environment daemons, run the following command: 
...
$ service sshd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000243','C-46176r1_chk','Only FIPS-approved ciphers should be used. To verify that only FIPS-approved ciphers are in use, run the following command: 
...
select add_rule_data ('default','V-38618','SV-50419r2_rule','default','low','The avahi service must be disabled.','Because the Avahi daemon service keeps an open network port, it is subject to network attacks. Its functionality is convenient but is only appropriate if the local network can be trusted.','F-43567r2_fix','The "avahi-daemon" service can be disabled with the following commands: 
 ...
 service avahi-daemon stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000246','C-46177r1_chk','Run the following command to verify "avahi-daemon" is disabled through current runtime configuration:
...
If the service is running, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38619','SV-50420r2_rule','default','medium','There must be no .netrc files on the system.','Unencrypted passwords for remote FTP servers may be stored in ".netrc" files. DoD policy requires passwords be encrypted in storage and not used in access scripts.','F-43569r2_fix','The ".netrc" files contain logon information used to auto-logon into FTP servers and reside in the user"s home directory. These files may contain unencrypted passwords to remote FTP servers making them susceptible to access by unauthorized users and should not be used. Any ".netrc" files should be removed.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000347','C-46179r3_chk','To check the system for the existence of any ".netrc" files, run the following command: 
...
$ service ntpd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000248','C-46180r1_chk','Run the following command to determine the current status of the "ntpd" service: 
...
$ service ntp start
$ chkconfig ntpd on' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000247','C-46178r1_chk','To verify the "ntp" package is installed on the system, run the following command: 
...
1        5        cron.daily        run-parts --report /etc/cron.daily' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000138','C-46183r1_chk','Run the following commands to determine the current status of the "logrotate" configuration.
...
Contact LDAP administrator and configure LDAP over TLS/SSL.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000252','C-46184r1_chk','If the system does not use LDAP for authentication or account information, this is not applicable.
...
Then review the LDAP server and ensure TLS has been configured.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000253','C-46185r1_chk','If the system does not use LDAP for authentication or account information, this is not applicable.
...
select add_rule_data ('default','V-38628','SV-50429r2_rule','default','medium','The operating system must produce audit records containing sufficient information to establish the identity of any user/subject associated with the event.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43576r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 chkconfig --level 2345 auditd on' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000145','C-46186r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000257','C-46188r3_chk','If the GConf2 package is not installed, this is not applicable.
...
' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000258','C-46189r3_chk','If the GConf2 package is not installed, this is not applicable.
...
select add_rule_data ('default','V-38631','SV-50432r2_rule','default','medium','The operating system must employ automated mechanisms to facilitate the monitoring and control of remote access methods.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43580r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 chkconfig --level 2345 auditd on' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000148','C-46190r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
select add_rule_data ('default','V-38632','SV-50433r2_rule','default','medium','The operating system must produce audit records containing sufficient information to establish what type of events occurred.','Ensuring the "auditd" service is active ensures audit records generated by the kernel can be written to disk, or that appropriate actions will be taken if other obstacles exist.','F-43581r2_fix','The "auditd" service is an essential userspace component of the Linux Auditing System, as it is responsible for writing audit records to disk. The "auditd" service can be enabled with the following commands: 
 ...
 chkconfig --level 2345 auditd on' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000154','C-46191r1_chk','Run the following command to determine the current status of the "auditd" service: 
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000165','C-46194r2_chk','To determine if the system is configured to audit calls to the "adjtimex" system call, run the following command:
...
$ service auditd reload' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000159','C-46195r1_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine how many logs the system is configured to retain after rotation: 
...
$ service auditd reload' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000160','C-46192r1_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine how much data the system will retain in each audit log file: 
...
','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38680','SV-50481r1_rule','default','medium','The audit system must identify staff members to receive notifications of audit log storage volume capacity issues.','Email sent to the root account is typically aliased to the administrators of the system, who can take appropriate action.','F-43629r1_fix','The "auditd" service can be configured to send email to a designated account in certain situations. Add or correct the following line in "/etc/audit/auditd.conf" to ensure that administrators are notified via email for those situations: 
...
$ service auditd reload' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000161','C-46193r3_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine if the system is configured to rotate logs when they reach their maximum size:
...
Set this value to "single" to cause the system to switch to single-user mode for corrective action. Acceptable values also include "suspend" and "halt". For certain systems, the need for availability outweighs the need to log all actions, and a different setting should be determined. Details regarding all possible values for [ACTION] are described in the "auditd.conf" man page. ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000163','C-54997r2_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine if the system is configured to either suspend, switch to single-user mode, or halt when disk space has run low:
...
$ service auditd reload' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000511','C-46020r1_chk','Inspect "/etc/audit/auditd.conf" and locate the following line to determine if the system is configured to take appropriate action when disk errors occur:
...
OR 
 ...
 yum reinstall [affected_package]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000281','C-46196r3_chk','The following command will list which audit files on the system have file hashes different from what is expected by the RPM database. 

 rpm -V audit | awk "$1 ~ /..5/ ANDAND $2 != "c""
...
select add_rule_data ('default','V-38638','SV-50439r3_rule','default','medium','The graphical desktop environment must have automatic lock enabled.','Enabling the activation of the screen lock after an idle period ensures password entry will be required in order to access the system, preventing access by passersby.','F-43587r1_fix','Run the following command to activate locking of the screensaver in the GNOME desktop when it is activated: 
 ...
 gconftool-2 --direct --config-source xml:readwrite:/etc/gconf/gconf.xml.mandatory --type bool --set /apps/gnome-screensaver/lock_enabled true' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000259','C-46198r3_chk','If the GConf2 package is not installed, this is not applicable. 
...
select add_rule_data ('default','V-38639','SV-50440r3_rule','default','low','The system must display a publicly-viewable pattern during a graphical desktop environment session lock.','Setting the screensaver mode to blank-only conceals the contents of the display from passersby.','F-43588r2_fix','Run the following command to set the screensaver mode in the GNOME desktop to a blank screen: 
 ...
 gconftool-2 --direct --config-source xml:readwrite:/etc/gconf/gconf.xml.mandatory --type string --set /apps/gnome-screensaver/mode blank-only' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000260','C-46199r4_chk','If the GConf2 package is not installed, this is not applicable. 

To ensure the screensaver is configured to be blank, run the following command: 

$ gconftool-2 --get /apps/gnome-screensaver/mode

If properly configured, the output should be "blank-only". 
If it is not, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38640','SV-50441r2_rule','default','low','The Automatic Bug Reporting Tool (abrtd) service must not be running.','Mishandling crash data could expose sensitive information about vulnerabilities in software executing on the local machine, as well as sensitive information from within a process"s address space or registers.','F-43589r2_fix','The Automatic Bug Reporting Tool ("abrtd") daemon collects and reports crash data when an application crash is detected. Using a variety of plugins, abrtd can email crash reports to system administrators, log crash reports to files, or forward crash reports to a centralized issue tracking system such as RHTSupport. The "abrtd" service can be disabled with the following commands: 

 chkconfig --level 0123456 abrtd off
 service abrtd stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000261','C-46200r1_chk','Run the following command to verify "abrtd" is disabled through current runtime configuration: 
...
select add_rule_data ('default','V-38641','SV-50442r2_rule','default','low','The atd service must be disabled.','The "atd" service could be used by an unsophisticated insider to carry out activities outside of a normal login session, which could complicate accountability. Furthermore, the need to schedule tasks with "at" or "batch" is not common.','F-43590r2_fix','The "at" and "batch" commands can be used to schedule tasks that are meant to be executed only once. This allows delayed execution in a manner similar to cron, except that it is not recurring. The daemon "atd" keeps track of tasks scheduled via "at" and "batch", and executes them at the specified time. The "atd" service can be disabled with the following commands: 
 ...
 chkconfig atd off
 service atd stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000262','C-46201r2_chk','If the system uses the "atd" service, this is not applicable.
...
select add_rule_data ('default','V-38644','SV-50445r2_rule','default','low','The ntpdate service must not be running.','The "ntpdate" service may only be suitable for systems which are rebooted frequently enough that clock drift does not cause problems between reboots. In any event, the functionality of the ntpdate service is now available in the ntpd program and should be considered deprecated.','F-43593r2_fix','The functionality of the ntpdate is now available as ntp daemon so it is considered deprecated hence remove the ntpdate package.
 ...
 rpm -e --nodeps ntpdate' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000265','C-46204r1_chk','Run the following command to verify "ntpdate" is installed on the system.
...
UMASK 077' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000345','C-46205r1_chk','Verify the "umask" setting is configured correctly in the "/etc/login.defs" file by running the following command: 
...
umask 077' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000344','C-46207r1_chk','Verify the "umask" setting is configured correctly in the "/etc/profile" file by running the following command: 
...
umask 077' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000343','C-46209r1_chk','Verify the "umask" setting is configured correctly in the "/etc/csh.cshrc" file by running the following command: 
...
If the above command returns no output, or if the umask is configured incorrectly, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38650','SV-50451r2_rule','default','low','The rdisc service must not be running.','General-purpose systems typically have their network and routing information configured statically by a system administrator. Workstations or some special-purpose systems often use DHCP (instead of IRDP) to retrieve dynamic network configuration information.','F-43599r2_fix','The "rdisc" service implements the client side of the ICMP Internet Router Discovery Protocol (IRDP), which allows discovery of routers on the local subnet. If a router is discovered then the local routing table is updated with a corresponding default route. By default this daemon is disabled. The "rdisc" service can be disabled with the following commands: 

 chkconfig rdisc off
 service rdisc stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000268','C-46210r1_chk','Run the following command to verify "rdisc" is disabled through current runtime configuration:
...
umask 077' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000342','C-46211r1_chk','Verify the "umask" setting is configured correctly in the "/etc/bashrc" file by running the following command: 
...
select add_rule_data ('default','V-38653','SV-50454r1_rule','default','high','The snmpd service must not use a default password.','Presence of the default SNMP password enables querying of different system aspects and could result in unauthorized knowledge of the system.','F-43602r1_fix','Edit "/etc/snmp/snmpd.conf", remove default community string "public". Upon doing that, restart the SNMP service: 
 ...
 service snmpd restart' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000341','C-46213r1_chk','To ensure the default password is not set, run the following command: 

 grep -v "^" /etc/snmp/snmpd.conf| grep public

There should be no output. 
If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38654','SV-50455r2_rule','default','medium','Remote file systems must be mounted with the nosuid option.','NFS mounts should not present suid binaries to users. Only vendor-supplied suid executables should be installed to their default location on the local filesystem.','F-43603r1_fix','Add the "nosuid" option to the mount option column of "/etc/fstab" for the line which controls mounting of any NFS mounts.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000270','C-46214r3_chk','To verify the "nosuid" option is configured for all NFS mounts, run the following command: 
...
[]" and then run mount [devicename] to effect the changes.]
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000271','C-46216r1_chk','To verify that binaries cannot be directly executed from removable media, run the following command:
...
Requiring samba clients such as "smbclient" to use packet signing ensures they can only communicate with servers that support packet signing.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000272','C-46217r1_chk','To verify that Samba clients running smbclient must use packet signing, run the following command: 
...
See the "mount.cifs(8)" man page for more information. A Samba client should only communicate with servers who can support SMB packet signing.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000273','C-46218r4_chk','If Samba is not in use, this is not applicable.
...
The requirement is 24 passwords.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000274','C-46219r1_chk','To verify the password reuse setting is compliant, run the following command: 
...
$ /etc/init.d/snmpd reload' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000340','C-46215r1_chk','To ensure only SNMPv3 or newer is used, run the following command: 
...
https://docs.redhat.com/docs/en-US/Red_Hat_Enterprise_Linux/6/html/Security_Guide/sect-Security_Guide-LUKS_Disk_Encryption.html' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000276','C-46221r1_chk','Determine if encryption must be used to protect data on the system. 
If encryption must be used and is not employed, this is a finding.','Customized rule by Cloud Raxak') from dual;
...
Detailed information on encrypting partitions using LUKS can be found on the Red Had Documentation web site:
https://docs.redhat.com/docs/en-US/Red_Hat_Enterprise_Linux/6/html/Security_Guide/sect-Security_Guide-LUKS_Disk_Encryption.html' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000277','C-46222r1_chk','Determine if encryption must be used to protect data on the system. 
If encryption must be used and is not employed, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38663','SV-50464r1_rule','default','medium','The system package management tool must verify permissions on all files and directories associated with the audit package.','Permissions on audit binaries and configuration files that are too generous could allow an unauthorized user to gain privileges that they should not have. The permissions set by the vendor should be maintained. Any deviations from this baseline should be investigated.','F-43612r1_fix','The RPM package management system can restore file access permissions of the audit package files and directories. The following command will update audit files with permissions different from what is expected by the RPM database: 

 rpm --setperms audit' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000278','C-46223r1_chk','The following command will list which audit files on the system have permissions different from what is expected by the RPM database: 
...
If there is any output, for each file or directory found, compare the RPM-expected permissions with the permissions on the file or directory:
 ...
 rpm -q --queryformat "[]" audit | grep  [filename]
 ls -lL [filename]

If the existing permissions are more permissive than those expected by RPM, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38664','SV-50465r1_rule','default','medium','The system package management tool must verify ownership on all files and directories associated with the audit package.','Ownership of audit binaries and configuration files that is incorrect could allow an unauthorized user to gain privileges that they should not have. The ownership set by the vendor should be maintained. Any deviations from this baseline should be investigated.','F-43613r1_fix','The RPM package management system can restore file ownership of the audit package files and directories. The following command will update audit files with ownership different from what is expected by the RPM database: 

 rpm --setugids audit' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000279','C-46224r1_chk','The following command will list which audit files on the system have ownership different from what is expected by the RPM database: 

 rpm -V audit | grep "^.....U"


If there is output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38665','SV-50466r1_rule','default','medium','The system package management tool must verify group-ownership on all files and directories associated with the audit package.','Group-ownership of audit binaries and configuration files that is incorrect could allow an unauthorized user to gain privileges that they should not have. The group-ownership set by the vendor should be maintained. Any deviations from this baseline should be investigated.','F-43614r1_fix','The RPM package management system can restore file group-ownership of the audit package files and directories. The following command will update audit files with group-ownership different from what is expected by the RPM database: 

 rpm --setugids audit' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000280','C-46225r1_chk','The following command will list which audit files on the system have group-ownership different from what is expected by the RPM database: 
...
Configure the virus scanning software to perform scans dynamically on all accessed files. If this is not possible, configure the system to scan all altered files on the system on a daily basis. If the system processes inbound SMTP mail, configure the virus scanner to scan all received mail. ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000284','C-46226r2_chk','
...
$ aide --init' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000285','C-46227r1_chk','Inspect the system to determine if intrusion detection software has been installed. Verify the intrusion detection software is active by using selinux or aide tools.
...
AIDE can be executed periodically through other means; this is merely one example.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000306','C-46229r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
select add_rule_data ('default','V-38671','SV-50472r1_rule','default','medium','The sendmail package must be removed.','The sendmail software was not developed with security in mind and its design prevents it from being effectively contained by SELinux. Postfix should be used instead.','F-43620r1_fix','Sendmail is not the default mail transfer agent and is not installed by default. The "sendmail" package can be removed with the following command: 
 ...
 rpm -e sendmail --nodeps' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000288','C-46231r1_chk','Run the following command to determine if the "sendmail" package is installed: 

 rpm -q sendmail


If the package is installed, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38672','SV-50473r2_rule','default','low','The netconsole service must be disabled unless required.','The "netconsole" service is not necessary unless there is a need to debug kernel panics, which is not common.','F-43622r2_fix','The "netconsole" service is responsible for loading the netconsole kernel module, which logs kernel printk messages over UDP to a syslog server. This allows debugging of problems where disk logging fails and serial consoles are impractical. The "netconsole" service can be disabled with the following commands: 

 chkconfig --level 0123456 netconsole off
 service netconsole stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000289','C-46233r1_chk','Check the configuration of netconsole kernel module.
...
Output should indicate the "netconsole" service has either not been installed, or has been disabled at all runlevels, as shown in the example below: 
 ...
 chkconfig netconsole --list 2>/dev/null
...
AIDE can be executed periodically through other means; this is merely one example.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000307','C-46232r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
id:3:initdefault:' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000290','C-46234r1_chk','To verify the default runlevel is 3, run the following command: 
...
* hard core 0' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000308','C-46235r2_chk','To verify that core dumps are disabled for all users, run the following command:
...
select add_rule_data ('default','V-38676','SV-50477r2_rule','default','low','The xorg-x11-server-common (X Windows) package must not be installed, unless required.','Unnecessary packages should not be installed to decrease the attack surface of the system.','F-43625r1_fix','Removing all packages which constitute the X Window System ensures users or malicious software cannot start X. To do so, run the following command: 
 ...
 rpm -e xorg-x11-server-common --nodeps"' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000291','C-46236r1_chk','To ensure the X Windows package group is removed, run the following command: 
...
GATEWAY=[local LAN default gateway]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000292','C-46242r2_chk','To verify that DHCP is not being used, examine the following file for each interface. 
...
If no line is returned, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38683','SV-50484r1_rule','default','low','All accounts on the system must have unique user or account names','Unique usernames allow for accountability on the system.','F-43632r1_fix','Change usernames, or delete accounts, so each has a unique name.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000296','C-46245r1_chk','Run the following command to check for duplicate account names: 
...
A documented site-defined number may be substituted for 10 in the above.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000319','C-46246r2_chk','Run the following command to ensure the "maxlogins" value is configured for all users on the system:
...
"[YYYY-MM-DD]" indicates the documented expiration date for the account.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000297','C-46247r1_chk','For every temporary account, run the following command to obtain its account aging and expiration information: 
...
Make sure you configure it to persist after reboot.
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000320','C-46248r1_chk','Check if IPv6 module is enabled or not using the following command :
...
If the default FORWARD policy is not set to DROP, this is a finding. 
    ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38687','SV-50488r2_rule','default','low','The system must provide VPN connectivity for communications over untrusted networks.','Providing the ability for remote users or systems to initiate a secure VPN connection protects information when it is transmitted over a wide area network.','F-43636r1_fix','The Openswan package provides an implementation of IPsec and IKE, which permits the creation of secure tunnels over untrusted networks. The "openswan" package can be installed with the following command: 

 yum install openswan' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000321','C-46249r2_chk','If the system does not communicate over untrusted networks, this is not applicable.
...
$ sudo gconftool-2 --direct --config-source xml:readwrite:/etc/gconf/gconf.xml.mandatory --type bool --set /apps/gdm/simple-greeter/disable_user_list true' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000527','C-49197r4_chk','If the GConf2 package is not installed, this is not applicable.

To ensure the user list is disabled, run the following command:

$ gconftool-2 --direct --config-source xml:readwrite:/etc/gconf/gconf.xml.mandatory --get /apps/gdm/simple-greeter/disable_user_list
...
To display a banner, this setting must be enabled and then banner text must also be set.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000324','C-46250r3_chk','If the GConf2 package is not installed, this is not applicable.
...
When entering a warning banner that spans several lines, remember to begin and end the string with """. This command writes directly to the file "/var/lib/gdm/.gconf/apps/gdm/simple-greeter/%gconf.xml", and this file can later be edited directly if necessary.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000326','C-46252r3_chk','If the GConf2 package is not installed, this is not applicable.
...
"[YYYY-MM-DD]" indicates the documented expiration date for the account.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000298','C-46251r1_chk','For every emergency account, run the following command to obtain its account aging and expiration information: 
...
select add_rule_data ('default','V-38691','SV-50492r2_rule','default','medium','The Bluetooth service must be disabled.','Disabling the "bluetooth" service prevents the system from attempting connections to Bluetooth devices, which entails some security risk. Nevertheless, variation in this risk decision may be expected due to the utility of Bluetooth connectivity and its limited range.','F-43640r1_fix','The "bluetooth" service can be disabled with the following command: 
 ...
 service bluetooth stop' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000331','C-46253r3_chk','To check that the "bluetooth" service is disabled in system boot configuration, run the following command: 
...
A value of 35 is recommended. If a password is currently on the verge of expiration, then 35 days remain until the account is automatically disabled. However, if the password will not expire for another 60 days, then 95 days could elapse until the account would be automatically disabled. See the "useradd" man page for more information. Determining the inactivity timeout must be done with careful consideration of the length of a "normal" period of inactivity for users in the particular environment. Setting the timeout too low incurs support costs and also has the potential to impact availability of the system to legitimate users.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000334','C-46254r2_chk','To verify the "INACTIVE" setting, run the following command: 
...
password required pam_cracklib.so maxrepeat=3 ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000299','C-46255r1_chk','To check the maximum value for consecutive repeating characters, run the following command: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000302','C-46257r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000303','C-46258r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 
...
To set the sticky bit on a world-writable directory [], run the following command: 
 ...
 chmod +t [DIR]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000336','C-46259r4_chk','To find world-writable directories that lack the sticky bit, run the following command for each local partition []: 
...
AIDE can be executed periodically through other means; this is merely one example.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000304','C-46261r2_chk','To determine that periodic AIDE execution has been scheduled, run the following command: 

 grep aide /etc/crontab /etc/cron.*/*

If there is no output, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38700','SV-50501r2_rule','default','medium','The operating system must provide a near real-time alert when any of the organization defined list of compromise or potential compromise indicators occurs. ','By default, AIDE does not install itself for periodic execution. Periodically running AIDE may reveal unexpected changes in installed files.','F-43649r1_fix','Install a real time monitoring tool on the system in order to recieve real time alerts when an indicator of compromise occurs. Please ensure whatever tool you install does not violate the organizational policy. You can install "ossec" which is a free and open source real time monitoring tool that also provides real time alerts.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000305','C-46262r2_chk','Check whether the operating system has any tool that provides a real time alert when any of the organization defined list of compromise or potential compromise indicators occurs.
If there is no such tool, this is a finding.','Customized rule by Cloud Raxak') from dual;
...
$ service xinetd  reload' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000338','C-46263r1_chk','Verify "tftp" is configured by with the "-s" option by running the following command: 

 grep -w server_args /etc/xinetd.d/tftp | grep -v "^"
...
log_ftp_protocol=YES' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000339','C-46264r1_chk','Find if logging is applied to the ftp daemon. 
...
Another keyboard sequence may be substituted for "<Control><Alt>l", which is the default for the Gnome desktop.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000508','C-46030r2_chk','If the GConf2 package is not installed, this is not applicable.

Verify the keybindings for the Gnome screensaver:

 gconftool-2 --get /apps/gnome_settings_daemon/keybindings/screensaver

If no output is visible, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38567','SV-50368r4_rule','default','low','The audit system must be configured to audit all use of setuid and setgid programs.','Privileged programs are subject to escalation-of-privilege attacks, which attempt to subvert their normal role of providing some necessary but limited capability. As such, motivation exists to monitor these programs for unusual activity.','F-43515r6_fix','At a minimum, the audit system should collect the execution of privileged commands for all users and root. To find the relevant setuid / setgid programs, run the following command for each local partition []:
...
Now, to get recently added audit rules into effect run the following command:
 ...
 auditctl -R /etc/audit/audit.rules
   ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000198','C-46125r7_chk','To verify that auditing of privileged command use is configured, run the following command once for each local partition [] to find relevant setuid / setgid programs:
...
select add_rule_data ('default','V-38443','SV-50243r1_rule','default','medium','The /etc/gshadow file must be owned by root.','The "/etc/gshadow" file contains group password hashes. Protection of this file is critical for system security.','F-43388r1_fix','Run the below command to change the ownership of "/etc/gshadow" to root:: 
 ...
 chown root /etc/gshadow' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000036','C-45998r1_chk','To check the ownership of "/etc/gshadow", run the command: 
...
If it does not, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38445','SV-50245r2_rule','default','medium','Audit log files must be group-owned by root.','If non-privileged users can write to audit logs, audit trails can be modified or destroyed.','F-43390r1_fix','Change the group owner of the audit log file(s) with the following command: 

 chgrp root []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000522','C-46000r1_chk','Run the following command to check the group owner of the system audit log file(s): 
...
select add_rule_data ('default','V-38448','SV-50248r1_rule','default','medium','The /etc/gshadow file must be group-owned by root.','The "/etc/gshadow" file contains group password hashes. Protection of this file is critical for system security.','F-43393r1_fix','To change the group ownership of "/etc/gshadow" to root, run the command: 

 chgrp root /etc/gshadow' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000037','C-46003r1_chk','To check the group ownership of "/etc/gshadow", run the command: 
...
select add_rule_data ('default','V-38449','SV-50249r1_rule','default','medium','The /etc/gshadow file must have mode 0000.','The /etc/gshadow file contains group password hashes. Protection of this file is critical for system security.','F-43394r1_fix','To properly set the permissions of "/etc/gshadow", run the command: 
 ...
 chmod 0000 /etc/gshadow' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000038','C-46004r1_chk','To check the permissions of "/etc/gshadow", run the command: 
...
select add_rule_data ('default','V-38450','SV-50250r1_rule','default','medium','The /etc/passwd file must be owned by root.','The "/etc/passwd" file contains information about the users that are configured on the system. Protection of this file is critical for system security.','F-43395r1_fix','Run the below command to change the ownership of "/etc/passwd" to root:: 
 ...
 chown root /etc/passwd' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000039','C-46005r1_chk','To check the ownership of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38451','SV-50251r1_rule','default','medium','The /etc/passwd file must be group-owned by root.','The "/etc/passwd" file contains information about the users that are configured on the system. Protection of this file is critical for system security.','F-43396r1_fix','To change the group ownership of "/etc/passwd" to root, run the command: 
 ...
 chgrp root /etc/passwd' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000040','C-46006r1_chk','To check the group ownership of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38457','SV-50257r1_rule','default','medium','The /etc/passwd file must have mode 0644 or less permissive.','If the "/etc/passwd" file is writable by a group-owner or the world the risk of its compromise is increased. The file contains the list of accounts on the system and associated information, and protection of this file is critical for system security.','F-43397r1_fix','To properly set the permissions of "/etc/passwd", run the command: 
 ...
 chmod 0644 /etc/passwd' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000041','C-46007r1_chk','To check the permissions of "/etc/passwd", run the command: 
...
select add_rule_data ('default','V-38458','SV-50258r1_rule','default','medium','The /etc/group file must be owned by root.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43403r1_fix','Run the below command to change the ownership of "/etc/group" to root:: 
 ...
 chown root /etc/group' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000042','C-46013r1_chk','To check the ownership of "/etc/group", run the command: 
...
select add_rule_data ('default','V-38459','SV-50259r1_rule','default','medium','The /etc/group file must be group-owned by root.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43404r1_fix','To change the group ownership of "/etc/group" to root, run the command: 
 ...
 chgrp root /etc/group' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000043','C-46014r1_chk','To check the group ownership of "/etc/group", run the command: 
...
select add_rule_data ('default','V-38461','SV-50261r1_rule','default','medium','The /etc/group file must have mode 0644 or less permissive.','The "/etc/group" file contains information regarding groups that are configured on the system. Protection of this file is important for system security.','F-43406r1_fix','To properly set the permissions of "/etc/group", run the command: 
 ...
 chmod 644 /etc/group' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000044','C-46015r1_chk','To check the permissions of "/etc/group", run the command: 
...
If any file in these directories is found to be group-writable or world-writable, correct its permission with the following command: 
 ...
 chmod go-w []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000045','C-46019r4_chk','System-wide shared library files, which are linked to executables during process load time or run time, are stored in the following directories by default: 
...
If any file in these directories is found to be owned by a user other than root, correct its ownership with the following command: 
 ...
 chown root []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000046','C-46021r1_chk','System-wide shared library files, which are linked to executables during process load time or run time, are stored in the following directories by default: 
...
If any file in these directories is found to be group-writable or world-writable, change its permission with the following command: 
 ...
 chmod go-w []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000047','C-46024r3_chk','System executables are stored in the following directories by default: 
...
If any file [] in these directories is owned by non-root user, change its ownership with the following command:
 chown root [FILE]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000048','C-46027r1_chk','System executables are stored in the following directories by default: 
...
select add_rule_data ('default','V-38493','SV-50294r1_rule','default','medium','Audit log directories must have mode 0755 or less permissive.','If users can delete audit logs, audit trails can be modified or destroyed.','F-43440r1_fix','Change the mode of the audit log directories with the following command: 
 ...
 chmod go-w []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000385','C-46050r1_chk','Run the following command to check the mode of the system audit directories: 
...
select add_rule_data ('default','V-38495','SV-50296r1_rule','default','medium','Audit log files must be owned by root.','If non-privileged users can write to audit logs, audit trails can be modified or destroyed.','F-43443r1_fix','Change the owner of the audit log files with the following command: 
 ...
 chown root []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000384','C-46053r1_chk','Run the following command to check the owner of the system audit logs: 
...
select add_rule_data ('default','V-38498','SV-50299r1_rule','default','medium','Audit log files must have mode 0640 or less permissive.','If users can write to audit logs, audit trails can be modified or destroyed.','F-43445r1_fix','Change the mode of the audit log files with the following command: 
 ...
 chmod 0640 []' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000383','C-46055r1_chk','Run the following command to check the mode of the system audit logs: 
...
select add_rule_data ('default','V-38502','SV-50303r1_rule','default','medium','The /etc/shadow file must be owned by root.','The "/etc/shadow" file contains the list of local system accounts and stores password hashes. Protection of this file is critical for system security. Failure to give ownership of this file to root provides the designated owner with access to sensitive information which could weaken the system security posture.','F-43449r1_fix','Run the below command to change the ownership of "/etc/shawdow" to root:: 
 ...
 chown root /etc/shadow' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000033','C-46059r1_chk','To check the ownership of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38503','SV-50304r1_rule','default','medium','The /etc/shadow file must be group-owned by root.','The "/etc/shadow" file stores password hashes. Protection of this file is critical for system security.','F-43450r1_fix','To change the group owner of "/etc/shadow" to root, run the command: 
 ...
 chgrp root /etc/shadow' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000034','C-46060r1_chk','To check the group ownership of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38504','SV-50305r1_rule','default','medium','The /etc/shadow file must have mode 0000.','The "/etc/shadow" file contains the list of local system accounts and stores password hashes. Protection of this file is critical for system security. Failure to give ownership of this file to root provides the designated owner with access to sensitive information which could weaken the system security posture.','F-43451r1_fix','To properly set the permissions of "/etc/shadow", run the command: 
 ...
 chmod 0000 /etc/shadow' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000035','C-46061r2_chk','To check the permissions of "/etc/shadow", run the command: 
...
select add_rule_data ('default','V-38579','SV-50380r1_rule','default','medium','The system boot loader configuration file(s) must be owned by root.','Only root should be able to modify important boot parameters.','F-43527r1_fix','The file "/etc/grub.conf" should be owned by the "root" user to prevent destruction or modification of the file. To change the owner of "/etc/grub.conf" to root, run the command: 
 ...
 chown root /etc/grub.conf' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000065','C-46137r1_chk','To check the ownership of "/etc/grub.conf", run the command: 
...
If it does not, this is a finding.','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-38581','SV-50382r1_rule','default','medium','The system boot loader configuration file(s) must be group-owned by root.','The "root" group is a highly-privileged group. Furthermore, the group-owner of this file should not have any access privileges anyway.','F-43529r1_fix','The file "/etc/grub.conf" should be group-owned by the "root" group to prevent destruction or modification of the file. To change the group owner of "/etc/grub.conf" to root, run the command: 

 chgrp root /etc/grub.conf' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000066','C-46139r1_chk','To check the group ownership of "/etc/grub.conf", run the command: 
...
Boot partitions based on VFAT, NTFS, or other non-standard configurations may require alternative measures.' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000067','C-46141r2_chk','To check the permissions of /etc/grub.conf, run the command:
...
If the permissions are not 600 or more restrictive, run the following command to correct this:
 ...
 chmod 0600 [LOGFILE]' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000135','C-46181r2_chk','All rsyslog-generated log files must have mode 0600 or less permissive. These log files are determined by the second part of each Rule line in "/etc/rsyslog.conf" and typically all appear in "/var/log". For each log file [] referenced in "/etc/rsyslog.conf", run the following command to inspect the file"s permissions: 
...
SELINUX=enforcing' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000020','C-53703r1_chk','Check the file "/etc/selinux/config" and ensure the following line appears:

SELINUX=enforcing

 grep -w SELINUX=enforcing /etc/selinux/config | grep -v "^"

If SELINUX is not set to enforcing, this is a finding. ','Customized rule by Cloud Raxak') from dual;
...
Other policies, such as "mls", provide additional security labeling and greater confinement but are not compatible with many general-purpose use cases. ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000023','C-53711r1_chk','Check the file "/etc/selinux/config" and ensure the following line appears:

SELINUXTYPE=targeted

 grep -w SELINUXTYPE=targeted /etc/selinux/config | grep -v "^"

If it does not, this is a finding. ','Customized rule by Cloud Raxak') from dual;
select add_rule_data ('default','V-51379','SV-65589r1_rule','default','low','All device files must be monitored by the system Linux Security Module.','If a device file carries the SELinux type "unlabeled_t", then SELinux cannot properly restrict access to the device file. ','F-56179r1_fix','Device files, which are used for communication with important system resources, should be labeled with proper SELinux types. If any device files carry the SELinux type "unlabeled_t", investigate the cause and correct the file"s context. ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000025','C-53719r1_chk','To check for unlabeled device files, run the following command:

 find /dev -exec stat --format=%n,%C {} \; |grep unlabeled_t |awk -F"," "{print $1}"
...
session required pam_lastlog.so showfailed' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000372','C-54013r1_chk','To ensure that last logon/access notification is configured correctly, run the following command:

 grep "session\s*required\s*pam_lastlog.so" /etc/pam.d/login | grep -v "^\s*"

 grep -w "^\s*session" /etc/pam.d/system-auth | grep -w required | grep -w pam_lastlog.so
...
[]
            ' , '[rpm-based Linux] DISA - Mission Critical Classified (Full)','Profile Customized by CloudRaxak','RHEL-06-000528','C-58279r1_chk','
